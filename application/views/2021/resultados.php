...
<!--CARD MODELO -->
              ...
              <div class="card my-1 slideInIp animated">
                ...
                <div class="card-header zoomIn animated">
                  <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                      <a class="nav-link " href="<?=base_url()?>futebol" id="tit_mkt_now" style="font-weight: bold;">Próximos Jogos mais correspondidos</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link active" href="<?=base_url()?>futebol/resultados" id="" style="font-weight: bold;">Últimos resultados</a>
                    </li>
                ...
                <div class="card-body fadeInLeft animated">
                  ...
                  <div class="row">
                    <form action="<?=base_url()?>futebol/resultados" method="post">
                      <? if(isset($_POST['dia'])){ $dia = $this->input->post('dia'); }else{ $dia = ""; } ?>
                        <input type="date" name="dia" id="dia_q" value="<?=$dia?>" />
                ...
                <table class="table table-hover table-borded" id="tb_rel" data-order='[[ 0, "asc" ]]' data-page-length='200' id="tb_rel" style="width: 100%">
            ...
            $posicao = 0; foreach($qr->result() as $dd){ $posicao++; 
              //print_r($dd);
                  ...
                  <? 
                    ...
                    ?>  
                        <tr class="bounceInUp animated" style="width: 100%;">
                          ...
                          <td><? #=$dd->inicio?><?=$dd->dt_betfair?></td>

                          <td><?=$dd->evento?></td>
                          <td> --
                            <?
                            #$qr_mkt_type1 = $this->padrao_model->get_by_matriz('id_mkt',$dd->id_mkt,'selections_types');
                            #$qr_mkt_type2 = $this->padrao_model->get_by_matriz('id_mkt',$dd->id_mercado,'selections_types');
                            ?>
                            <!--
                            <? #=$dd->id_mkt?> (<? #=$qr_mkt_type1->num_rows()?>) <br /> <? #=$dd->id_mercado?> (<? #=$qr_mkt_type2->num_rows()?>)-->
                            <br>
                            <? #=print_r($qr_mkt_type2->result())?>
                            
                          </td>
                          <td class="badge badge-success badge-pill" style="margin-top:0.5em "><?=$dd->winner?></td>
                        </tr>
                    <? #} // x if ?>
                    <? } // x foreach ?>
