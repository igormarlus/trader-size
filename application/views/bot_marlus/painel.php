<!doctype html>
<html dir="ltr" lang="pt-br">
  <head>
    <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <!-- Bootstrap CSS -->

  <link rel="stylesheet" href="<?=base_url()?>jack/node_modules/bootstrap/compilados/bootstrap.css">
   

  <link rel="stylesheet" href="https://daneden.github.io/animate.css/animate.min.css">

<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.10.19/css/jquery.dataTables.min.css">
 
    
  <meta charset="UTF-8">
 
    <title>Painel BOT Marlus</title>

   
    <style type="text/css">
      .bt_mkt:active{
        background-color: #ffb900;
      
}      .bt_mkt:hover{
        background-color: #ffb900;
      }

      .bt_mkt .card-body:hover{
        background-color: #ff0000;
      }

      .bgbtmkt:hover{
        background-color: #ffb900;
      }

      .bgbtmkt:active{
        background-color: #ffb900;
      }

       

    </style>





  </head>



  <body>
  



   
<?php #include("includes/2019/header.php");?>
<!-- NAVBAR END--->

<!-- BREADCRUMB -->
<style type="text/css">
  <?  if($this->agent->is_mobile()){ ?>
    .breadcrumb-item,.breadcrumb-item a{font-size: 11px}
  <? } ?>

</style>

<!-- END BREADCRUMB -->




<!-- CONTAINER SECTION -->
      <div class="container pb-3 pt-3 bg-white">




<!-- CONTAINER MAIN -->
        <div class="container p-0">

          <div class="row">

            <div class="container pl-1 pr-1"> <!-- LINHA -->
              <div class="col-lg-12 col-sm-12 col-12 order-sm-1 border-top"></div>
            </div>


<!-- END COLUNA-1 -->

<!-- COLUNA 2 -->
            <div class="col-lg-12 col-sm-12 col-md-6 order-xl-3 order-md-3 order-sm-2 pl-1 pr-1 text-justify">

              <ul class="nav nav-tabs">
                <li class="active"><a href="#">Home</a></li>
                <li class="dropdown">
                  <a class="dropdown-toggle" data-toggle="dropdown" href="#">Menu 1
                  <span class="caret"></span></a>
                  <ul class="dropdown-menu">
                    <li><a href="#">Submenu 1-1</a></li>
                    <li><a href="#">Submenu 1-2</a></li>
                    <li><a href="#">Submenu 1-3</a></li>
                  </ul>
                </li>
                <li><a href="#">Menu 2</a></li>
                <li><a href="#">Menu 3</a></li>
              </ul>

              

            
<!--CARD MODELO -->
              <!-- MATCH ODDS -->


              <div class="card my-1 slideInUp animated">

                <div class="card-header flipIn animated">
                  <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                      <button class="nav-link btn btn-success" href="#" id="get_bet_mo" style="font-weight: bold;">Pegar Mercado MATCH_ODDS </button>
                    </li>

                    <li class="nav-item">
                      <button class="nav-link btn btn-dark" id="get_odd_mo" style="font-weight: bold;"> Pegar odds MATCH ODDS</button>
                    </li>

                    

                     <li class="nav-item">
                      <a class="nav-link btn btn-warning" href="<?=base_url()?>bot_marlus/clear_mkt" style="font-weight: bold;"> Reset (Limpa mercados)</a>
                    </li>

                    
                  </ul>
                  <br><hr>
                  <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                      <button class="nav-link btn btn-success" href="#" id="get_bet_cc" style="font-weight: bold;">Pegar Mercado CORRECT SCORE </button>
                    </li>

                    <li class="nav-item">
                      <button class="nav-link btn btn-dark" id="get_odd_cc" style="font-weight: bold;"> Pegar odds CORRECT SCORE</button>
                    </li>

                    

                     <li class="nav-item">
                      <a class="nav-link btn btn-warning" href="<?=base_url()?>bot_marlus/clear_mkt" style="font-weight: bold;"> Reset (Limpa mercados)</a>
                    </li>

                    
                  </ul>
                  <br>
                  <br>


                  <div class="card my-1" id="display_call" style="max-height: 500px"> ... </div>

              
                                  



            





                </div>

                <div class="card my-1">
                  
                  <a class="btn btn-success" href="<?=base_url()?>bot_odds/filtro"> HOJE </a>
                  
                </div>

               <? if($mkts->num_rows() > 0){ ?>
                    <table class="table table-hover table-responsive" id="tb_rel" data-order='[[ 0, "asc" ]]' data-page-length='200'>
                      <thead>
                      <tr>
                          <th>ID</th>
                          <!--<th>Horário</th>-->
                          <th>Evento</th>
                          <th>Selection</th>
                          <th>Tipo</th>
                          <th>Odd</th>
                          <th>Status</th>
                          <!--
                          <th>Percurso</th>
                          <th>Local</th>
                          <th>País</th>
                          <th>Cavalos</th>
                          <th>Odd</th> -->
                      </tr>
                      </thead>

                      <?
                      $cont=0;
                      foreach($mkts->result() as $mkt){
                        $cont++;
                        //print_r($run);
                          $qr_selections = $this->padrao_model->get_by_matriz('id_mkt',$mkt->id_mkt,'selections_types'); 
                          #echo "<h1>".$qr_mkt->num_rows()."</h1>";
                           if($qr_selections->num_rows() > 0){?>

                            <? foreach($qr_selections->result() as $sels){  ?>
                              <tr class="bt_mais" style="cursor: pointer;" title="<?=$mkt->id_mkt?>">
                                <td><?=$cont?></td>
                                <td>
                                  <button class="bt_filter_mkt" value="<?=$mkt->id_mkt?>"><?=$sels->id_mkt?> </button>
                                </td>
                                <td>
                                  
                                    <?=$this->padrao_model->get_by_matriz('id_selection',$sels->id_selection,'selections')->row()->name?> 
                                  
                                </td>
                                <td><?=$sels->tipo?></td>
                                <td><?=$sels->odd?></td>
                                <td>
                                  <?if($sels->win == 1){?>
                                    <label class="alert alert-success">Venceu</label>
                                  <? }else{ ?>
                                    <label class="alert alert-danger">Perdeu</label>
                                  <? } ?>
                                    
                                  </td>

                               
                              </tr>
                            <? } ?>







                       <?   /* $qr_sels = $this->db->query("SELECT DISTINCT selection_id, selection_name FROM odds_mkt_horses WHERE id_mkt = '".$run->marketId."' ORDER BY odd asc "); 
                                foreach($qr_sels->result() as $sel){
                                  if($run->countryCode == "GB" || $run->countryCode == "US" ){
                      ?>
                                      <tr style="display: ;" class="mais<?=$run->id?> fadeInLeft animated">
                                        <th><?=$run->id?></th>
                                        <th>
                                          <?=$run->open_date?>
                                        </th>
                                        <td><?=$run->nome?></td>
                                        <td><?=$run->marketName?></td>
                                        <th><?=$run->local?></th>
                                        <th><?=$run->countryCode?></th>
                                        <th><?=$sel->selection_name?></th>
                                        <th><?=$this->db->query("SELECT odd FROM odds_mkt_horses WHERE selection_id = ".$sel->selection_id." AND tipo = 'back' ORDER BY  dt_update desc LIMIT 1")->row()->odd?></th>
                                      </tr>
                                  <? } // x if GB IRE ?>
                          <? } */ // x qr sel ?>


                          <? } // x if num_rows)() ?>
                      <? } // x foreach ?>

                    </table>
                  <? } ?>

               
                 
                
   

            
                

              </div>


            </div>






        </div>

<!-- END CONTAINER MAIN -->

      </div>

<!-- END CONTAINER SECTION -->


<?php include("includes/2019/footer.php");?>

    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="<?=base_url()?>jack/node_modules/jquery/dist/jquery.js"></script>
    
    <script src="<?=base_url()?>jack/node_modules/popper.js/dist/umd/popper.js"></script>
  
    <script src="<?=base_url()?>jack/node_modules/bootstrap/dist/js/bootstrap.js"></script>
  


<!-- JS  CRIADOS -->
<script type="text/javascript">
function testAnim(e,x) {
	$('#'+e).removeClass().addClass(x + ' animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
	  $(this).removeClass();
	});
};	



  $(document).ready(function(){

    $("#get_bet_mo").click(function(){

      $("#display_call").html("Aguarde...");

      $.get("<?=base_url()?>botbet/get_mkt_evento/MATCH_ODDS" , function(data){
        $("#display_call").html(data);
      })

    })

    $("#get_bet_cc").click(function(){

      $("#display_call").html("Aguarde...");

      $.get("<?=base_url()?>botbet/get_mkt_evento/CORRECT_SCORE" , function(data){
        $("#display_call").html(data);
      })

    })

   $(".clique").click(function(){
     var id_ani = $(this).attr('id');
     //alert(id_ani);
      testAnim(id_ani,'jello');
      //return false;
   })

  }) // x ready

  </script>


  <script type="text/javascript" language="javascript" src="https://code.jquery.com/jquery-3.3.1.js"></script>
  <script type="text/javascript" language="javascript" src="https://cdn.datatables.net/1.10.19/js/jquery.dataTables.min.js"></script>

  <script type="text/javascript" class="init">
  

$(document).ready(function() {

    $("#get_odd_mo").click(function(){

      $("#display_call").html("Aguarde...");

      
        setInterval(function(){ 
          //$("#display_call").html("");
          //alert("Hello"); 
          $.get("<?=base_url()?>cron/get_odds/0" , function(data){
            $("#display_call").html(data);
          })

        }, 
        10000);
        

  });

  $("#get_odd_cc").click(function(){

      $("#display_call").html("Aguarde...");

      
        setInterval(function(){ 
          //$("#display_call").html("");
          //alert("Hello"); 
          $.get("<?=base_url()?>cron/get_odds/121" , function(data){
            $("#display_call").html(data);
          })

        }, 
        10000);
        

  });  


/*
  // Setup - add a text input to each footer cell
  $('#td_rel tfoot th').each( function () {
    var title = $(this).text();
    $(this).html( '<input type="text" placeholder="Search '+title+'" />' );
  } ); */

  // DataTable
  var table = $('#tb_rel').DataTable(function(){

    $(".bt_filter_mkt").click(function(){
      var id_mkt_f = $(this).val();
      //alert(id_mkt_f);
      $("#tb_rel_filter input").val(id_mkt_f);
      $("#tb_rel_filter input").focus();

      $("#tb_rel_filter input").keyup();

    });

  });

  $(".bt_filter_mkt").click(function(){
    var id_mkt_f = $(this).val();
    //alert(id_mkt_f);
    $("#tb_rel_filter input").val(id_mkt_f);
    $("#tb_rel_filter input").focus();

    $("#tb_rel_filter input").keyup();

  });
  

  // Apply the search
  /*
  table.columns().every( function () {
    var that = this;

    $( 'input', this.footer() ).on( 'keyup change', function () {
      if ( that.search() !== this.value ) {
        that.search( this.value ).draw();
      }
    } );
  } );*/
  
  

} );


  </script>

  </body>
</html>