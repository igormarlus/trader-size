...
<link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/edit-dash.css">
    ...
    <title>Xbitcompany - Upgrades</title>
</head>
                ...
                <div class="admin-content-main">
                    <div class="admin-content-main-inner">
                        <div class="container-fluid">
                                <div class="box">
                                	<h1 class="page-header mb60">Relatório de upgrade dos usuários</h1>
    
   								<? if($upgrades->num_rows() == 0){ ?>
                                	<h3 class="page-header mb60">Nenhum Upgrade registrado</h3>
                                <? }else{ ?>
                                ...
                                <table class="table table-simple">
                                    ...
                                    <tbody>
                                    	<? 
										foreach($upgrades->result() as $up){ 
										$user = $this->padrao_model->get_by_id($up->id_user,'user')->row();
										?>
                                    		<tr>                                            
                                                <td class="semi-bold"><?=$user->id?></td>
                                                <td><?=$user->login?></td>
                                                <td> <?=$this->padrao_model->get_by_id($user->patrocinador,'user')->row()->login?></td>
                                                <td> Para: <?=$this->padrao_model->get_by_id($up->id_plano,'user_plano')->row()->nome?></td>
                                                <td>
													<?=$this->padrao_model->converte_data(substr($up->dt,0,10))?>  --
													<?=substr($up->dt,10,10)?>
                                                </td>
                                                <td class="min-width nowrap">
                                                <? if($up->status == '0'){ ?>
                                                	<a href="<?=base_url()?>dash/set_plano_adm/<?=$user->id?>/<?=$up->id_plano?>" class="btn-standard">Confirmar</a>
                                                <? }else{ ?>    
