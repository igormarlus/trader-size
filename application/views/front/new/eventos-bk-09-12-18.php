								...
								-->
										...
										<h3 style="">
											...
											<a itemprop="relatedLink" title="<?=$dd_evento->name?>"  href="<?=base_url()?>bets/jogo_by_mkt/<?=$dd->marketId?>">
												...
												<? #=str_replace(" v "," x ","aaaa")?>	
												<strong style="font-size:"><?=$times[1]?> x <?=$times[2]?> </strong>
											</a>
										...
										<span style="" class="">
												<?  /*if( $qr_partida->num_rows() > 0 ){  $dd_evento =  $qr_partida->row();   ?>
													<a itemprop="relatedLink" title="<? #=$dd_evento->id_evento?>" href="<?=base_url()?>futebol/jogo/<?=url_title($dd_evento->evento)?>/<?=$dd_evento->id_evento?>" class="btn btn-success">
														$<?=number_format($dd->totalMatched, 2, ',', '.')?>
														
													</a>
													<? }else{ */?>
														
													<label itemprop="relatedLink" title="<?=$times[1]?> x <?=$times[2]?>"  class="label label-success" style="font-size:20px">$<?=number_format($dd->totalMatched, 2, ',', '.')?>
