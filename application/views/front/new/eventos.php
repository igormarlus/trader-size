...
<head>
	...
	<meta charset="UTF-8">
	<? if($this->uri->segment(1) == "futebol"){ ?>
	<title>Próximos Jogos  Futebol  - Trader Esportivo Futebol  </title>

    <meta name="title" content="Próximos Jogos Futebol  - Trader Esportivo Futebol -  Previsões de Jogos " />
    <meta name="description" content="Futebol  Apostas Esportivas Futebol. Próximos Jogos Mercados mais correspondidos. Previsões de Jogos "  />
    <? } ?>

    <? if($this->uri->segment(1) == "bets"){ ?>

    	<title>Next Games  Futbol  - Trader Sporting Futbol  </title>

    <meta name="title" content="Next Games  Futbol  - Trader Sporting Futbol" />
    <meta name="description" content="Futebol  Bets sporting Futbol. Next Games Marketing more Matched. Previsões de Jogos "  />
...
<link rel="apple-touch-icon" sizes="180x180" href="<?=base_url()?>imagens/icons/touch/logo-168.png">
...
<!-- DEFINICOES DE ATALHO -->
<link rel="apple-touch-icon" sizes="57x57" href="<?=base_url()?>imagens/icons/touch/logo-48.png"/>
<link rel="apple-touch-icon" sizes="72x72" href="<?=base_url()?>imagens/icons/touch/logo-72.png"/>
<link rel="apple-touch-icon" sizes="114x114" href="<?=base_url()?>imagens/icons/touch/logo-144.png"/>

<link rel="apple-touch-icon-precomposed" sizes="57x57" href="<?=base_url()?>imagens/icons/touch/logo-48.png"/>
<link rel="apple-touch-icon-precomposed" sizes="72x72" href="<?=base_url()?>imagens/icons/touch/logo-72.png"/>
<link rel="apple-touch-icon-precomposed" sizes="114x114" href="<?=base_url()?>imagens/icons/touch/logo-144.png"/>

<link rel="apple-touch-startup-image" href="<?=base_url()?>imagens/icons/touch/logo-72.png">

<meta name="apple-mobile-web-app-capable" content="yes" />
<meta name="apple-mobile-web-app-status-bar-style" content="black" />

<link rel="canonical" href="<?=base_url()?><?=$this->uri->segment(1)?>">
				...
				<div class="container clearfix">
					<div class="col_three_fifth bothsidebar nobottommargin">
						<div class="fancy-title title-border">
							<? if($this->uri->segment(1) == "futebol"){ ?>
								<h1 itemprop="keywords"><a href="<?=base_url()?>futebol">Próximos Jogos Futebol</a></h1>
							<? } ?>
							<? if($this->uri->segment(1) == "bets"){ ?>
								<h1 itemprop="keywords"><a href="<?=base_url()?>futebol">Next Games</a></h1>
							<? } ?>
						...
						<div id="posts" class="events small-thumbs">
							<? if($this->uri->segment(1) == "futebol"){ ?>
								<h5 align="right" style="color:silver;text-align: right;">Jogos Mais Correspondidos - TOP 20</h5>
							<? } ?>
							<? if($this->uri->segment(1) == "bets"){ ?>
								<h5 align="right" style="color:silver;text-align: right;">Marketing More Matched - TOP 20</h5>
							<? } ?>
							  ...
							  <table class="table">
								<thead>
								  <tr>
										<!--<th>Mercado</th>-->
										<? if($this->uri->segment(1) == "futebol"){ ?>
										<th><strong style="color:#5bc0de">Jogo</strong></th>
										<th><strong style="color:#5bc0de">Correspondido</strong></th>						
										<? } ?>
										<? if($this->uri->segment(1) == "bets"){ ?>
										<th><strong style="color:#5bc0de">Event</strong></th>
										<th><strong style="color:#5bc0de">Matched</strong></th>						
										<? } ?>		  
									</tr>
								</thead>
								<tbody>
						...
						$posicao = 0; foreach($this->betfront_model->getMarketings_best($APP_KEY, $SESSION_TOKEN,1,20,'MATCH_ODDS') as $dd){ $posicao++; 
							...
							#		$times = explode(' v ',$dd_evento->evento); 
								...
								<tr>
									<td>								
										<h2 style="color: :black;font-size: 14px;margin-bottom:0px" data-animate="flash">
											<a itemprop="relatedLink" class="link jogos_lk" title="<?=$dd_evento->name?>"  href="<?=base_url()?>bets/jogo_by_mkt/<?=$dd->marketId?>">
												<strong style="font-size:11px;color:black" class="jogos_lk">
													<?=$times[1]?> x <?=$times[2]?> 
												</strong>
											</a>
										</h2>
									...
									<td>
										<span style="" class="" >
													<label itemprop="relatedLink" title="<?=$times[1]?> x <?=$times[2]?>"  class=""  style="color:green;font-size: 10px" data-animate="rotateInUpRight"> 
														<strong>$<?=number_format($dd->totalMatched, 2, ',', '.')?></strong>
													</label>
											</span>
						...
						-->
                            ...
                            <img width="500px" alt="Betfair" src="<?=base_url()?>imagens/banner/banner-betfair.jpg" border=0 style="display:" data-animate="swing" ></img >
                            </a>
						</div>
...
Acompanhe os volumes dos mercados através de gráficos e porcentagens atualizados em tempo real, melhorando sua intuição de analisar rapidamente as situações dos eventos.
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".jogos_lk").click(function(){
				$(this).attr('style','color:#ECD078');
