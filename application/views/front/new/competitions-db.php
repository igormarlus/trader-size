...
<head>
    ...
    <meta name="description" content="Apostas Esportivas Online Futebol <?=$competition->nome?> <? foreach($jogos->result() as $jogo_meta){ ?><?=$jogo_meta->evento?> | <? } ?>" />
...
<body class="stretched">
		...
		"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
		{
...

			{
				"@type":"ListItem","position":2,
					"item":{
						"@id":"<?=base_url()?>futebol/competition/<?=url_title($competition->nome)?>/<?=$competition->id_competition?>","name":"Próximos Jogos <?=$competition->nome?> "}
					}
			]}
	</script>
							...
							<script type="application/ld+json">
	{
	  ...
	  "@type":"BroadcastEvent",
	  "name": "Apostas - Bets: <?=str_replace(" v "," x ",$dd->evento)?>",
	  ...
	  "broadcastOfEvent": {
	    "@type": "SportsEvent",
	    "name": "<?=str_replace(" v "," x ",$dd->evento)?>",
	    "description": "<?=str_replace(" v "," x ",$dd->evento)?> - <?=$competition->nome?> Apostas Futebol Online",
	    //"availability": 10,
	    //"price" : 100,
	    ...
	    "performer": {
		    ...
		    "name": "Betfair"
							...
							<div class="entry clearfix">
								...
								<div class="entry-image hidden-sm">
									<a href="<?=base_url()?>bets/jogo/<?=url_title($jogo->evento)?>/<?=$jogo->id_evento?>">
										<img src="<?=base_url()?>images/parallax/home/1.jpg" alt="<?=$jogo->evento?>">
										<div class="entry-date"><?=substr($jogo->inicio,8,2)?>
											<span><?=$this->padrao_model->mes(substr($jogo->inicio,5,2))?></span>
										</div>
									</a>
								</div>
								...
								<div class="entry-c">
									<div class="entry-title">
										<h2 itemscope itemtype="https://schema.org/WebPage"><a target="_blank" href="<?=base_url()?>bets/jogo/<?=url_title($jogo->evento)?>/<?=$jogo->id_evento?>" itemprop="relatedLink"><?=str_replace(" v "," x ",$jogo->evento)?></a></h2>
									</div>
									<ul class="entry-meta clearfix">
										<li><span class="label label-success">Aberto</span></li>
										<li><a href="#"><i class="icon-time"></i> <?=$jogo->inicio?></a></li>
