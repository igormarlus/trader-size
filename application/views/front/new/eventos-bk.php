					...
					<div class="col_three_fifth bothsidebar nobottommargin">
						...
						$posicao = 0; foreach($this->betfront_model->getMarketings_best($APP_KEY, $SESSION_TOKEN,1,20,'MATCH_ODDS') as $dd){ $posicao++; 
							...
							#		$times = explode(' v ',$dd_evento->evento); 
								...
								?>
								<tr>
									<td><strong title="<?=$dd_evento->timezone?>"><?=$dd_evento->openDate?></strong></td>
									<td>
									...
									<td>
										<h3 style="font-size: 14px">
											...
											<a itemprop="relatedLink" title="<?=$dd_evento->name?>"  href="<?=base_url()?>futebol/jogo/<?=url_title($dd_evento->name)?>/<?=$dd_evento->id?>">
												<?=str_replace(" v "," x ",$dd_evento->name)?>	
											</a> 
									...
									<td>
										...
										<a itemprop="relatedLink" title="<? #=$dd_evento->id_evento?>" href="<?=base_url()?>futebol/jogo/<?=url_title($dd_evento->name)?>/<?=$dd_evento->id?>" class="btn btn-success">Veja Mais</a>
