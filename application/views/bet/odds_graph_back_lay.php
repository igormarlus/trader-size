		...
		$selectionId = $selection['selection_id'];
				?>
                    ...
                    <?
							...
							$qr_selection = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'lay' ORDER BY id asc  LIMIT $inicio,10 ")or die(mysql_error()); 
							
							$qr_selection_vol = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'lay' ORDER BY id asc LIMIT 500 ")or die(mysql_error()); 
					?>
                            <!--- DADOS BACK-->
                     ...
                     ########################### LAY
                        ...
                        #if($L == 1){
                        $qr_selection_lay = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'back' ORDER BY id desc LIMIT 10 ")or die(mysql_error());  
                        $qr_selection_vol_lay = mysql_query("SELECT * FROM odds_mkt WHERE  id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'back' ORDER BY id desc LIMIT 500 ")or die(mysql_error());    
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="dashboard-box bg-white content-box">
                                                    ...
                                                    <div class="button-pane">
                                                        <div class="size-md float-left heading">
                                                        <?
                                                        $soma = mysql_query("SELECT SUM(tamanho) as soma FROM odds_mkt  WHERE id_mkt = ".$id_mkt." AND selection_id = ".$selectionId." AND tipo = 'lay' order by id desc LIMIT 500  ");
                                                        $soma_total = mysql_fetch_assoc($soma);
                                                        ?>
                                                            $<?=number_format($soma_total['soma'], 2, ',', '.')?>
                                                        </div>
