			...
			function printAvailablePrices_odd($selectionId, $marketBook,$id_mkt,$id_jogo)
			{
				...
				foreach ($marketBook->runners as $runner) 
                    ...
                    <tr style=''>
                        ...
                        <th style="text-align:center">Odds 
                        	<a target="_blank" style="color:#000;float: right;" href="<?=base_url()?>bet/clear_mkt/<?=$id_mkt?>">  <i class="glyph-icon icon-refresh" title="Limpar Dados do Mercado"></i>
								               </a>  </th>
...
*/
                            ...
                            <td>
                            	<div class='box_escada'>
                            		...
                            		<table cellpadding="0" cellspacing="0" border="0" id="" class="table table-striped table-bordered tb_rel escada_odds" data-order='[[ 0, "desc" ]]' data-page-length='50'>
                                <thead class="dd_old">
                                <tr>
                                	<th>Lucro Back</th>
                                    <th>Back</th>
                                ...
                                <tbody class="dd_old" id="load_tr">	
									...
									while($dd_vert_back = mysql_fetch_assoc($qr_v_back)){
										...
										$nm_class_lay = "o_".$selectionId.'_'.$odd_trat.'l';
									?>

		                            	<? if($dd_vert_back['odd'] == $odd_atual_back){ 
		                            		$bg_color_back = '0083ca'; 
		                            		$bg_color_now = '000;color:#fff'; 
		                            	}else{ 
		                            		$bg_color_back = 'B7DBFF'; 
		                            		$bg_color_now = 'FFF;color:#000'; 
		                            	}; ?>
		                            	<tr>
		                            		<td style="font-size: 10px">$0,00</td>
		                            		 
		                            		<td style="padding:2px;text-align:center;color:#000;border:black 1px solid;background-color:#B7DBFF;width:80px;font-size:10px;height:auto" class="set_odd basic-dialog back"  title="<?=$selectionId?>"  lang="<?=$dd_vert_back['odd']?>" ><small class="volume<?=$nm_class?>"><?=$dd_vert_back['tamanho']?></small></td>
			                            	<td style="padding:2px;text-align:center;color:#000;border:black 1px solid;background-color:#<?=$bg_color_now?>;width:80px;font-size:10px;height:auto" class="set_odd basic-dialog back <?=$nm_class?>" title="<?=$selectionId?>"  lang="<?=$availableToBack->price;?>">
													
			                                <label class='preco'><?=$dd_vert_back['odd']?> <? #=$nm_class?> </label>
			                                <!--
											<br>
											<small class="volume<?=$nm_class?>"><?=$dd_vert_back['tamanho']?></small>-->
			                                </td>
			                                <td style="padding:2px;text-align:center;color:#000;border:black 1px solid;background-color:#FFD5EA;width:80px;font-size:10px;height:auto" class="set_odd basic-dialog lay <?=$nm_class_lay?> volume<?=$nm_class_lay?>" title="<?=$selectionId?>"  lang="<?=$dd_vert_back['odd']?>"></td>
			                                <td style="font-size: 10px">$0,00</td>
					...
					*/
					...
					if(mysql_num_rows($qr_num) == 0){
                        	...
                        	<td valign="top">
                        		<div class='box_escada'>
		                            ...
		                            <? if($dd_vert_lay['odd'] == $odd_atual_lay){ $bg_color_lay = 'f9677e'; }else{ $bg_color_lay = 'FFD5EA'; }; ?>
		                            	<div style="padding:5px;text-align:center;color:#000;border:black 1px solid;background-color:#<?=$bg_color_lay?>;width:80px;font-size:10px;height:auto" class='set_odd basic-dialog lay <?=$nm_class?>'  title="<?=$selectionId?>" lang="<?=$dd_vert_lay['odd']?>">
                        ...
                        -->
                            ...
                            <td style='background-color:#f9677e;text-align:center;color:#000;vertical-align:text-top;padding:5px;width:60px' class='set_odd basic-dialog lay'  title="<?=$selectionId?>" lang="<?=$availableToBack->price;?>">
				...
				<div class="col-md-6 tb_partidas" style='border:red 0px solid;margin-bottom:20px;display:'> 
                            ...
                            <table class='tb_partidas table table-bordered' style='border: #ffb80c 1px solid;padding:10px;border-radius: 20px;margin-bottom: 15px '> 
								<thead class="cf">
									<tr style=''>
					                    	<th colspan="7">
					                    		<h3 id="sel<?=$runner->selectionId?>" class='tb_partidas title-hero' title="<?=$runner->selectionId?>" style="border-radius:0px;border-bottom:#ffb80c 1px solid;width:auto;font-weight: bold;"><?=$runner->runnerName?></h3>
					                    	</th> 
	...
	*/
	$(document).ready(function() {
      // var table =  $('.tb_rel').dataTable();
	   var table = $('.escada_odds').dataTable( {
					paging: false,
					select: true,
					scrollX: true,
					searching: false,
	   ...
	   $('.escada_odds tbody').on( 'click', 'tr', function () {
