							...
							function mostrar($odd, $marketBook,$id_mkt,$id_jogo)
							{
								...
								function printAvailablePrices_odd($selectionId, $marketBook,$id_mkt,$id_jogo)
								{
									...
									foreach ($marketBook->runners as $runner) 
									   ...
									   <!-- <td>-->
										<div class="example-box-wrapper">
											<div class="row">
												...
												if( $pecentual_back > 85 && ($availableToBack->price > 1.3 && $availableToBack->price < 1.67)){ 
													$cont_bet = rand(5, 15);
													...
													$qr_hot = mysqli_query($con,"SELECT * FROM odds_hot WHERE `id_mkt` = '".$id_mkt."' AND `selection_id` = '".$selectionId."' AND id_user = '".constant("id_user_session")."' ");
													if(mysqli_num_rows($qr_hot) == 0 ){

														$qr_sel = mysqli_query($con,"SELECT * FROM odds_mkt WHERE id_mkt = '".$id_mkt."' AND  selection_id = '".$selectionId."' AND selection_name <> '' "); 	
														...
														if ($selecao != "" ) {
														  ...
														  <script type="text/javascript">
																$(document).ready(function(){
																	...
																	$("#entradas").append("<p class='alert alert-warning'><a href='https://www.betfair.com/exchange/plus/football/market/<?=$id_mkt?>' target='_blank'>Entrar no mercado <?=$id_mkt?> </a> <br>$selectionId = <?=$selectionId?> (<?=$selecao?>)** [<?=$bet?>] <?=$pecentual_back?>% <span class='bet<?=$cont_bet?>'>**</span> </p> <br />");
																	...
																	var stake = $("#stake").val();
																	//alert(stake);
																	$(".bet<?=$cont_bet?>").html("Aguardando...");
																	$.post('<?=base_url()?>bet/place_bet' , {'id_mkt' : <?=$id_mkt?> , 'id_selection' : <?=$selectionId?> , 'tipo' : 'back', 'size' : 1.3 , 'valor' : stake } , function(data){
														...
														mysqli_query($con,"INSERT INTO `odds_hot` (`id_user`,`id_partida`,`id_mkt`, `side`, `selection_id`, `selection_name`, `odd`,`tamanho` , `resultado` , `status` ) 
																				 VALUES ( ".constant("id_user_session")." , ".$id_jogo." ,'".$id_mkt."', 'back', '".$selectionId."', 'Time', '".$availableToBack->price."' , ".$pecentual_back." , 33 , '1')")or die(mysqli_error($con));

														/*

														mysqli_query($con,"INSERT INTO `odds_hot` (`id_user`,`id_partida`,`id_mkt`, `side`, `selection_id`, `selection_name`, `odd`,`tamanho` , `resultado` ) 
																				 VALUES ( ".constant("id_user_session")." , ".$id_jogo." ,'".$id_mkt."', 'back', '".$selectionId."', 'Time', '".$availableToBack->price."' , 50 , 22)")or die(mysqli_error($con));
														*/
										...
										*/
										include("includes/mysqli_con.php");form_upload('name');
				...
				*/
								...
								*/
								...
								foreach ($odd->runners as $runner) {
									mysqli_query($con,"UPDATE `odds_mkt` SET `selection_name` = '".$runner->runnerName."' , `total_matched` = '".$odd->totalMatched."' , `bg` = '' , `diferenca` = '0' WHERE selection_id = '".$runner->selectionId."' AND id_mkt = '".$id_mkt."'  ORDER BY dt_update desc LIMIT 1");
								?>	
...
<? ########################################### TOTAL MATCHED
			/*$this->ftp->upload($this->ftp->upload($local, $remote, 'auto', 0775);, $remote, 'auto', 0775);
