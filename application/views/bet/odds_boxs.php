...
#		$this->bet_model->printMarketIdRunnersAndPrices($odd, $marketBook,$id_mkt);
							...
							function mostrar($odd, $marketBook,$id_mkt,$id_jogo)
							{
								...
								function printAvailablePrices_odd($selectionId, $marketBook,$id_mkt,$id_jogo)
								{
									...
									foreach ($marketBook->runners as $runner) 
										...
										$qr_num = mysqli_query($con,"SELECT id_mkt FROM odds_mkt WHERE id_mkt = '".$id_mkt."' AND  selection_id = '".$selectionId."' AND odd = ".$availableToBack->price." AND tipo = 'back' ")or die(mysqli_error($con));	
										...
										if(mysqli_num_rows($qr_num) == 0){
											...
											mysqli_query($con,"UPDATE `odds_mkt` SET `tamanho` = ".$availableToBack->size." , `dt_update` = CURRENT_TIMESTAMP  WHERE selection_id = '".$selectionId."' AND odd = ".$availableToBack->price." AND tipo = 'back' AND id_mkt = '".$id_mkt."'" )or die(mysqli_error($con));
										}
										...
										<div class="example-box-wrapper">
											<div class="row">
												...
												if($availableToBack->price > 1.1 && $availableToBack->price < 1.7){
													echo "<h1> Esse é ".$availableToBack->price."</h1>";
														...
														<div class="tile-box tile-box-alt  set_odd lay" lang="<?=$availableToLay->price;?>" title="<?=$selectionId?>">
															...
															<div class="tile-content-wrapper">
																...
																<div class="tile-content preco">
																	...
																	$qr_lasts_lay = mysqli_query($con,"SELECT * FROM `odds_mkt` WHERE selection_id = ".$selectionId." AND tipo = 'lay' AND id_mkt = ".$id_mkt." ORDER BY dt_update desc LIMIT 1,1 " )or die(mysql_error());
																	$dd_last_lay = mysqli_fetch_assoc($qr_lasts_lay);
																	#echo mysql_num_rows($qr_lasts);
																	#echo "<label style='font-size:12px'>".$dd_last['odd']."</label>";
		...
		$qr_hot = mysqli_query($con,"SELECT * FROM odds_hot WHERE `id_mkt` = '".$id_mkt."' AND `selection_id` = '".$selectionId."' AND `side` = 'lay' ");
				if(mysqli_num_rows($qr_hot) == 0 ){
					mysqli_query($con,"INSERT INTO `odds_hot` (`id_partida`,`id_mkt`, `side`, `selection_id`, `selection_name`, `odd`,`tamanho` ) 
											 VALUES ( ".$id_jogo." ,'".$id_mkt."', 'lay', '".$selectionId."', 'Time', '".$availableToLay->price."' , ".$pecentual_lay.")")or die(mysql_error());
				}else{
					/*
					...
					*/
								...
								include("includes/mysqli_con.php");
								$diferenca = 0;
								$last_match_qr = mysqli_query($con,"SELECT total_matched,bg,diferenca FROM odds_mkt WHERE id_mkt = '$id_mkt' ORDER BY 'dt','desc' LIMIT 1,1")or die(mysql_error());
								...
								foreach ($odd->runners as $runner) {
									mysqli_query($con,"UPDATE `odds_mkt` SET `selection_name` = '".$runner->runnerName."' , `total_matched` = '".$odd->totalMatched."' , `bg` = '".$bg."' , `diferenca` = '".$diferenca."' WHERE selection_id = '".$runner->selectionId."' AND id_mkt = '".$id_mkt."'  ORDER BY dt_update desc LIMIT 1");
								?>	
...
<div href="#" title="Example box" class="tile-box btn btn-warning tb_partidas">
	...
	<div class="tile-content-wrapper">
		...
		ECHO
			<? } */ 
			############################################################################################ INICIO CASH OUT
			echo "<h1>SESS ".$SESSION_TOKEN."</h1>";
			$entradas = $this->betfront_model->list_bets_by_mkt('sl4K5RkqJvpsKvPc', $SESSION_TOKEN,$id_mkt) ;
			$cont_entr_geral = 0;
			$cont_entr = -1;
			#echo count($entradas)." ++ ";
			#print_r($entradas);
...
<? ########################################### TOTAL MATCHED
			...
			$seta = "";
			$diferenca = 0;
            $last_match_qr = mysqli_query($con,"SELECT total_matched,bg,diferenca FROM odds_mkt WHERE id_mkt = '$id_mkt' ORDER BY 'dt','desc' LIMIT 1")or die(mysqli_error($con));
