...
$qr_distinct = mysql_query("SELECT DISTINCT selection_id, selection_name FROM odds_mkt WHERE id_mkt = $id_mkt AND tipo = 'back' AND  selection_name <> '' ");
	$cont_sel = 0;
    if(mysql_num_rows($qr_distinct) > 0){
		...
		$selectionId = $selection['selection_id'];
				?>
                    ...
                    <?
							...
							$qr_selection_num = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'lay'")or die(mysql_error()); 
							...
							$qr_selection = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'lay' ORDER BY dt_update asc  LIMIT $inicio,50 ")or die(mysql_error()); 
							
							$qr_selection_vol = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'lay' ORDER BY dt_update asc LIMIT 50 ")or die(mysql_error()); 
					?>
                            <!--- DADOS BACK-->
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="dashboard-box bg-blue">
                                                    <div class="content-wrapper">
                                                        <div class="header">
                                                            Odds Back  <strong title="Odds registradas">(<?=$selection_num?>)</strong>
                                            ...
                                            <div class="dashboard-box bg-white content-box">
                                                    ...
                                                    <div class="button-pane">
                                                        <div class="size-md float-left heading">
                                                        <?
														$soma = mysql_query("SELECT SUM(tamanho) as soma FROM odds_mkt  WHERE id_mkt = ".$id_mkt." AND selection_id = ".$selectionId." AND tipo = 'lay' order by id desc LIMIT 50  ");
...
$qr_distinct_lay = mysql_query("SELECT DISTINCT selection_id, selection_name FROM odds_mkt WHERE id_mkt = $id_mkt AND tipo = 'lay' AND  selection_name <> '' ");
	$cont_sel = 0;
	while ($selection = mysql_fetch_assoc($qr_distinct_lay)) {
	$selectionId = $selection['selection_id'];
					
						#if($L == 1){
						$qr_selection_lay = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'back' ORDER BY dt_update desc LIMIT 50 ")or die(mysql_error()); 	
						$qr_selection_vol_lay = mysql_query("SELECT * FROM odds_mkt WHERE  id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'back' ORDER BY dt_update desc LIMIT 50 ")or die(mysql_error()); 	
						
						$qr_selection_num_lay = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = $id_mkt AND selection_id = ".$selectionId." AND tipo LIKE 'back'")or die(mysql_error()); 
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="dashboard-box bg-white content-box">
                                                    ...
                                                    <div class="button-pane">
                                                        <div class="size-md float-left heading">
                                                        <?
														$soma = mysql_query("SELECT SUM(tamanho) as soma FROM odds_mkt  WHERE id_mkt = ".$id_mkt." AND selection_id = ".$selectionId." AND tipo = 'back' order by id desc LIMIT 10  ");
														$soma_total = mysql_fetch_assoc($soma);
                                                        ?>
                                                            $<?=number_format($soma_total['soma'], 2, ',', '.')?>
                                                        </div>
                                            ...
                                            <div class="col-md-6">
                                                <div class="dashboard-box bg-primary">
                                                    <div class="content-wrapper">
                                                        <div class="header">
                                                           Odds  Lay <strong title="Odds registradas">(<?=$selection_num_lay?>)</strong>
                                                            <span><?=$selection['selection_name']?></span>
                                                        </div>
                                                        <div class="sparkline-big">
														<? while ($row_vol_lay = mysql_fetch_assoc($qr_selection_lay)) {
															echo $row_vol_lay["odd"].",";
															$last = $row_vol_lay["odd"];
														   }
														   echo $last;
														?>
