...
<? if($this->uri->segment(1) == 'botbettttttt'){ ?>
...
#		$this->bet_model->printMarketIdRunnersAndPrices($odd, $marketBook,$id_mkt);
		#print_r($marketBook);
		echo "<h1>LIVE: ".$marketBook->inplay."</h1>";
		#return false;
							function mostrar($odd, $marketBook,$id_mkt,$id_jogo)
							{
								...
								function printAvailablePrices_odd($selectionId, $marketBook,$id_mkt,$id_jogo)
								{
									...
									if($marketBook->status != 'OPEN'){ // defini mercado suspenso ou fechado
										...
										if($id_mkt == 'CLOSED'){
												echo $id_mkt."***";
												#mysql_query("DELETE FROM odds_mkt_horses WHERE id_mkt = '".$id_mkt."' ")or die(mysql_error());
									...
									foreach ($marketBook->runners as $runner) 
										...
										$qr_num = mysqli_query($con,"SELECT id_mkt FROM odds_mkt_horses WHERE id_mkt = '".$id_mkt."' AND  selection_id = '".$selectionId."' AND odd = ".$availableToBack->price." AND tipo = 'back' ")or die(mysqli_error($con));	
										
										if(mysqli_num_rows($qr_num) == 0){
											mysqli_query($con,"INSERT INTO `odds_mkt_horses` (`id_partida`,`id_mkt`, `selection_id`, `tamanho`, `odd`, `tipo`, `atual` ,`dt`,`dt_update`) VALUES ('".$id_jogo."','".$id_mkt."', '".$selectionId."', '".$availableToBack->size."', '".$availableToBack->price."', 'back', '".$atual."',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)")or die(mysqli_error($con));
										}else{
											mysqli_query($con,"UPDATE `odds_mkt_horses` SET `tamanho` = ".$availableToBack->size." , `dt_update` = CURRENT_TIMESTAMP  WHERE selection_id = '".$selectionId."' AND odd = ".$availableToBack->price." AND tipo = 'back' AND id_mkt = '".$id_mkt."'" )or die(mysqli_error($con));
										}
										
										
											if($f == 1){

													$qr_verifica_reg = mysqli_query($con,"SELECT * FROM `corridas_cavalos_2020` WHERE selection_id = '".$selectionId."' ")or die(mysqli_error($con));

													if($marketBook->inplay == 1){

														if(mysqli_num_rows($qr_verifica_reg) == 0){

															echo "NOVO CADASTRO";
															mysqli_query($con,"INSERT INTO `corridas_cavalos_2020` (`id_mkt`, `selection_id`,`bsp`) VALUES ('".$id_mkt."', '".$selectionId."', ".$availableToBack->price." )")or die(mysqli_error($con));

														}else{
															$ln_ordem = mysqli_fetch_assoc($qr_verifica_reg);
															echo "ORDEM: <strong>".$ln_ordem['ordem']."</strong> ";


															// ESTRATÉGIA LAY AO FAVORITO
															#if( ($ln_ordem['ordem'] == 4 OR $ln_ordem['ordem'] == 5) && $availableToBack->price > 15){
															if( ($ln_ordem['ordem'] == 4 OR $ln_ordem['ordem'] == 5) ){
																include("includes/mysqli_con.php");

																echo "<h3>Essa é pra entrar</h3>";

																$qr_verifica_bet = mysqli_query($con,"SELECT * FROM `odds_hot_horses` WHERE id_mkt = '".$id_mkt."' AND selection_id = '".$selectionId."' ")or die(mysqli_error($con));
																if(mysqli_num_rows($qr_verifica_bet) == 0){


																mysqli_query($con,"INSERT INTO `odds_hot_horses` (`id_user`,`id_partida`,`id_mkt`, `side`, `selection_id`, `selection_name`, `odd`,`tamanho` , `resultado` ) 
																					 VALUES ( ".constant("id_user_session")." , 2020 ,'".$id_mkt."', 'LAY', '".$selectionId."', 'Time', '".$availableToBack->price."' , 50 , 2008)")or die(mysqli_error($con));
																} // x if numrows
															} // x if estrategia LAY AO FAVORITO
														} // x else
												...
												//limpa os atuais
											#	mysql_query("UPDATE `odds_mkt_horses` SET  `agora` = 0  WHERE selection_id = '".$selectionId."'  AND tipo = 'back' AND id_mkt = '".$id_mkt."'  " );
												...
												*/
												...
												#$qr_teste = $this->db->get('odds_mkt_horses');
												#$this->db->update('odds_mkt_horses',$dd_up_a);
												...
												#mysql_query("UPDATE `odds_mkt_horses` SET `atual` = '1' WHERE selection_id = '".$selectionId."' AND odd = ".$availableToBack->price." AND tipo = 'back'" );
												
												#$qr_selection = mysql_query("SELECT * FROM odds_mkt_horses WHERE selection_id = ".$selectionId." AND tipo LIKE 'back' ORDER BY id desc  LIMIT 10 ")or die(mysql_error()); 
												#$qr_selection_vol = mysql_query("SELECT * FROM odds_mkt_horses WHERE selection_id = ".$selectionId." AND tipo LIKE 'back' ORDER BY id desc LIMIT 10 ")or die(mysql_error()); 
										?>
												<!--- DADOS BACK-->
									   <!-- <td>-->
										<div class="example-box-wrapper">
											<div class="row">
												<?
												// SETA JOGO FAVORITO NA TB partidas_bet
												if($availableToBack->price > 1.1 && $availableToBack->price < 1.7){
													/*
													echo "<h1> Esse é FAVORITO ".$availableToBack->price."</h1>";

													$qr_verifica_reg = mysqli_query($con,"SELECT * FROM `partidas_bet` WHERE id_partida = '".$id_jogo."' ")or die(mysqli_error($con));
													if(mysqli_num_rows($qr_verifica_reg) == 0){

														echo "SETOU FAVORITO";
														mysqli_query($con,"INSERT INTO `partidas_bet` (`id_partida`,`id_mkt`, `selection_id`,`odd`,`tipo`) VALUES ('".$id_jogo."','".$id_mkt."', '".$selectionId."', ".$availableToBack->price." , '1' )")or die(mysqli_error($con));
													}else{
														echo "JA TEM";
													}
													*/
												...
												$soma_back_sel = mysqli_query($con,"SELECT SUM(tamanho) as soma FROM odds_mkt_horses  WHERE id_mkt = ".$id_mkt."  AND selection_id = ".$selectionId." AND tipo = 'lay' order by dt_update desc LIMIT 5  ");
												$soma_back = mysqli_query($con,"SELECT SUM(tamanho) as soma FROM odds_mkt_horses  WHERE id_mkt = ".$id_mkt." AND tipo = 'lay' order by dt_update desc LIMIT 5  ");
												...
												#echo number_format($pecentual_back, 2, ',', '.').'%';
													...
													$qr_hot = mysqli_query($con,"SELECT * FROM odds_hot WHERE `id_mkt` = '".$id_mkt."' AND `selection_id` = '".$selectionId."' AND `side` = 'back' ");
													if(mysqli_num_rows($qr_hot) == 0 ){
														mysqli_query($con,"INSERT INTO `odds_hot` (`id_partida`,`id_mkt`, `side`, `selection_id`, `selection_name`, `odd`,`tamanho` ) 
																				 VALUES ( ".$id_jogo." ,'".$id_mkt."', 'back', '".$selectionId."', 'Time', '".$availableToBack->price."' , ".$pecentual_back.")")or die(mysql_error());
													}else{
														/*
												...
												<div class="col-md-3">
														...
														<a href="#" title="Soma dos valores correspondidos nas Seleções BACK dividido pela quantidade de seleções BACK" class="tile-box tile-box-alt <?=$bg_class?>" style="height: 180px">
															...
															<div class="tile-content-wrapper" style="height: 110px">
										...
										*/
										include("includes/mysqli_con.php");
										$qr_num = mysqli_query($con,"SELECT id_mkt FROM odds_mkt_horses WHERE id_mkt = '".$id_mkt."' AND selection_id = '".$selectionId."' AND odd = ".$availableToLay->price." AND tipo = 'lay' ")or die(mysql_error()); 
										
										if(mysqli_num_rows($qr_num) == 0){
											mysqli_query($con,"INSERT INTO `odds_mkt_horses` (`id_partida`,`id_mkt`, `selection_id`, `tamanho`, `odd`, `tipo`, `atual`,`dt`,`dt_update`) VALUES ('".$id_jogo."','".$id_mkt."', '".$selectionId."', '".$availableToLay->size."', '".$availableToLay->price."', 'lay', '".$atual."',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)");
										}else{
								...
								include("includes/mysqli_con.php");
								$diferenca = 0;
								$last_match_qr = mysqli_query($con,"SELECT total_matched,bg,diferenca FROM odds_mkt_horses WHERE id_mkt = '$id_mkt' ORDER BY 'dt','desc' LIMIT 1,1")or die(mysql_error());
								...
								<? }else{ ?>
								   ...
								   <h3 class="content-box-header bg-red">
                                        ...
                                        if($marketBook->status == "CLOSED"){
                                        	include("includes/mysqli_con.php");
                                        mysqli_query($con,"DELETE FROM odds_mkt_horses WHERE id_mkt = '".$id_mkt."' ")or die(mysqli_error($con)); 
                                    	mysqli_query($con,"DELETE FROM mercados_horses WHERE id_mkt = '".$id_mkt."' ")or die(mysqli_error($con)); 
                                    	mysqli_query($con,"DELETE FROM runs_horses WHERE marketId = '".$id_mkt."' ")or die(mysqli_error($con)); 
								...
								foreach ($odd->runners as $runner) {
									mysqli_query($con,"UPDATE `odds_mkt_horses` SET `selection_name` = '".$runner->runnerName."' , `total_matched` = '".$odd->totalMatched."' , `bg` = '".$bg."' , `diferenca` = '".$diferenca."' WHERE selection_id = '".$runner->selectionId."' AND id_mkt = '".$id_mkt."'  ORDER BY dt_update desc LIMIT 1");
								?>	
		...
		<div class="tile-content">
			...
			foreach($entradas as $dd_entr){ $cont_entr++;
				...
				if($id_mkt_entr == $id_mkt){
					...
					$qr_lasts = mysql_query("SELECT * FROM `odds_mkt_horses` WHERE selection_id = ".$id_selection_entr." AND tipo = 'back' AND id_mkt = ".$id_mkt_entr." ORDER BY dt_update desc LIMIT 1,1 " )or die(mysql_error());
			...
			*/
			...
			for($b=0; $b<count($entradas); $b++){ #$tg++;
					...
					#$dd_bet_mkt = $this->db->get('odds_mkt_horses');
						...
						foreach($entradas as $dd_entr){ $cont_entr++;
							...
							for($e=0; $e<count($dd_entr); $e++){  #$tg++;
								...
								if($id_mkt_entr == $id_mkt){
									...
									$qr_lasts = mysql_query("SELECT * FROM `odds_mkt_horses` WHERE selection_id = ".$id_selection_entr." AND tipo = '".$side_cash."' AND id_mkt = ".$id_mkt_entr." ORDER BY dt_update desc LIMIT 1" )or die(mysql_error());
									...
									$nm_mkt_sel = $this->padrao_model->get_by_matriz('selection_id',$id_selection_entr,'odds_mkt_horses')->row()->selection_name;
									
									$nm_mkt_sel = $this->db->query(" SELECT * FROM odds_mkt_horses where selection_id = '".$id_selection_entr."' AND selection_name <> '' ")->row()->selection_name;
...
<? ########################################### TOTAL MATCHED
			...
			$seta = "";
			$diferenca = 0;
            $last_match_qr = mysqli_query($con,"SELECT total_matched,bg,diferenca FROM odds_mkt_horses WHERE id_mkt = '$id_mkt' ORDER BY 'dt','desc' LIMIT 1")or die(mysqli_error($con));
            if(mysqli_num_rows($last_match_qr) > 0){
				$last_match = mysqli_fetch_assoc($last_match_qr);
				if($last_match['total_matched'] > $odd->totalMatched){
					$bg = 'red';
					$seta = 'icon-caret-down';
