...
require_once('includes/betapi/jsonrpc-futbol.php'); 
	...
	$entrada = $this->bet_model->list_bets_by_mkt($APP_KEY, $SESSION_TOKEN,$id_mkt);
	#echo $APP_KEY.' - '.$SESSION_TOKEN.' - '.$id_mkt;
	#print_r($entrada);
	
	foreach($entrada as $dd_bet_now_ativo){ 
	#echo "Opa 2";
		#if(count($dd_bet_now_ativo) > 0){
			for($a=0; $a<count($dd_bet_now_ativo); $a++){ $tg++;
				if(isset($dd_bet_now_ativo[$a])){
					
					#$bet_idbet = $dd_bet_now_ativo[$a]->betId;
					#$bet_mkt_id = $dd_bet_now_ativo[$a]->marketId;
					$bet_selection_id_ativo = $dd_bet_now_ativo[$a]->selectionId;
					$bet_status_ativo = $dd_bet_now_ativo[$a]->status;
					$bet_side_ativo = $dd_bet_now_ativo[$a]->side;
					...
					$bet_price_ativo = $dd_bet_now_ativo[$a]->averagePriceMatched;
					$bet_size_ativo = $dd_bet_now_ativo[$a]->sizeMatched;
		...
		function mostrar($odd, $marketBook,$id_mkt)
		{
			...
			function printAvailablePrices_odd($selectionId, $marketBook,$id_mkt,$id_jogo)
			{
				...
				foreach ($marketBook->runners as $runner) 
                    ...
                    <thead class="cf">
                    <tr style=''>
                    	<th style="background-color:#0083ca;color:#fff">Back:
							<?
                        ...
                        <th style="text-align:center">Size</th>
                        <th style="background-color:#f9677e;color:#fff">Lay:
							<?
...
*/
                            ...
                            <td>
                            <?
                            $qr_v_back = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = '".$id_mkt."' AND selection_id = '".$selectionId."' AND tipo = 'back' ORDER BY id desc ")or die(mysql_error()); 
							...
							?>
                            	...
                            	<div style="padding:5px;text-align:center;color:#000;border:black 1px solid;background-color:#<?=$bg_color_back?>;width:80px;" class="set_odd basic-dialog back" title="<?=$selectionId?>"  lang="<?=$availableToBack->price;?>">
										
                                <label class='preco'><?=$dd_vert_back['odd']?></label>
								<br>
								<small><?=$dd_vert_back['tamanho']?></small>
                                </div>
					...
					*/
					...
					if(mysql_num_rows($qr_num) == 0){
						...
						?>
                        	<td valign="top">
                            <?
                            $qr_v_lay = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = '".$id_mkt."' AND selection_id = '".$selectionId."' AND tipo = 'lay' ORDER BY id desc ")or die(mysql_error()); 
                            ...
                            <? if($dd_vert_lay['odd'] == $odd_atual_lay){ $bg_color_lay = 'f9677e'; }else{ $bg_color_lay = 'FFD5EA'; }; ?>
                            	<div style="padding:5px;text-align:center;color:#000;border:black 1px solid;background-color:#<?=$bg_color_lay?>;width:80px" class='set_odd basic-dialog lay'  title="<?=$selectionId?>" lang="<?=$dd_vert_lay['odd']?>">
                                <label class='preco'><?=$dd_vert_lay['odd']?></label>
                                <br>
								<small><?=$dd_vert_lay['tamanho']?>  </small>
                                </div>
                            <? } ?>    
                            </td>
							<td style="color:red">
			...
			?>	
				<div class="col-md-3 tb_partidas" style='border:red 0px solid;margin-right:100px;display: block'> 
					<h3 id="sel<?=$runner->selectionId?>" class='tb_partidas title-hero' title="<?=$runner->selectionId?>"><?=$runner->runnerName?></h3>
						...
						?>
                            ...
                            <table class='tb_partidas table table-bordered' style='border:green 0px solid;display: block;'> 
