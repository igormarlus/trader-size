							...
							function mostrar($odd, $marketBook,$id_mkt,$id_jogo)
							{
								...
								function printAvailablePrices_odd($selectionId, $marketBook,$id_mkt,$id_jogo)
								{
									...
									foreach ($marketBook->runners as $runner) 
										...
										echo "<h1>".mysql_num_rows($qr_num)."</h1>";
										...
										if(mysql_num_rows($qr_num) == 0){
											mysql_query("INSERT INTO `odds_mkt` (`id_partida`,`id_mkt`, `selection_id`, `tamanho`, `odd`, `tipo`, `atual` ,`dt`,`dt_update`) VALUES ('".$id_jogo."','".$id_mkt."', '".$selectionId."', '".$availableToBack->size."', '".$availableToBack->price."', 'back', '".$atual."',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)")or die (mysql_error());
										}else{
									   ...
									   <!-- <td>-->
										<div class="example-box-wrapper">
											<div class="row">
												...
												if($availableToBack->price > 2.1 && $availableToBack->price < 2.6){
													...
													if(mysql_num_rows($qr_hot) == 0 ){
														...
														$qr_sel = mysql_query("SELECT * FROM odds_mkt WHERE id_mkt = '".$id_mkt."' AND  selection_id = '".$selectionId."' AND selection_name <> '' "); 	
														if(mysql_num_rows($qr_sel) == 0){
															$selecao = "indefinido";
														}else{
															$dd_sel = mysql_fetch_assoc($qr_sel);
														...
														if ($selecao == "Under 2.5 Goals" ) {
														  ...
														  <script type="text/javascript">
																$(document).ready(function(){
																	...
																	audio.play(); 
																	console.log(" bet ----- <?=$selecionId?>");
																	$("#entradas").append("<p class='alert alert-warning'><a href='https://www.betfair.com/exchange/plus/football/market/<?=$id_mkt?>' target='_blank'>Entrar no mercado <?=$id_mkt?> </a> <br>$selectionId = <?=$selectionId?> (<?=$selecao?>)** [<?=$bet?>] <span class='bet<?=$selecionId?>'>**</span> </p> <br />");
																	...
																	$.post('<?=base_url()?>bet/place_bet' , {'id_mkt' : <?=$id_mkt?> , 'id_selection' : <?=$selectionId?> , 'tipo' : 'back', 'size' : 2 , 'valor' : 3 } , function(data){
																			$(".bet<?=$selecionId?>").html(data);
														...
														mysql_query("INSERT INTO `odds_hot` (`id_partida`,`id_mkt`, `side`, `selection_id`, `selection_name`, `odd`,`tamanho` , `resultado` ) 
																				 VALUES ( ".$id_jogo." ,'".$id_mkt."', 'back', '".$selectionId."', 'Time', '".$availableToBack->price."' , ".$pecentual_back." , 22)")or die(mysql_error());
														...
														mysql_query("UPDATE `odds_hot` SET `odd` = '".$availableToBack->price."' ,  `view` = '0' , `tamanho` = ".$pecentual_back." WHERE `id_mkt` = '".$id_mkt."' AND `selection_id` = '".$selectionId."' AND `side` =  'back' ");
													}
												}
...
<div href="#" title="Example box" class="tile-box btn btn-warning tb_partidas" style='display: none'>
	...
	<div class="tile-content-wrapper" style="display: none;">
		...
		<div class="tile-content">
			...
			############################################################################################ INICIO CASH OUT
			#echo $SESSION_TOKEN." ** *  *";
			$entradas = $this->bet_model->list_bets_by_mkt('2dLZpzfBGNbApLMb', $SESSION_TOKEN,$id_mkt) ;
			$cont_entr_geral = 0;
			$cont_entr = -1;
			#echo count($entradas)." ++ ";
			#print_r($entradas);
