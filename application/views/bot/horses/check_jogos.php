...
<!-- COLUNA 2 -->
            <div class="col-lg-12 col-sm-12 col-md-6 order-xl-3 order-md-3 order-sm-2 pl-1 pr-1 text-justify">
              ...
              <div class="card my-1" id="">
                <a href="<?=base_url()?>horses/rel_runs" target="_blank" class="btn btn-primary">Pegar mais Corridas</a>
              </div>
               <div class="card my-1" id="">
                <a href="<?=base_url()?>cron/get_runs" target="_blank" class="btn btn-success">Mais Mercados</a>
              </div>
...
<!--CARD MODELO -->
              ...
              <div class="card my-1 slideInUp animated">
               ...
               <? */ ?>
                ...
                <? if($jogos->num_rows() == 0){ ?>
                    <h3 class="page-header mb60">Nenhuma corrida registrado</h3>
                <? }else{ ?>
                
                <!--  class="table tb_rel table-simple"-->
                 <form action="<?=base_url()?>horse_bot_hist/set_jogos"  method="post">
                 <p><input type="submit" value="Salvar" class="btn btn-success"></p>
                <table class="table table-hover table-responsive" id="tb_rel" data-order='[[ 7, "asc" ]]' data-page-length='200'>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        foreach($mercados->result() as $mkt){ 
                          ...
                          $qr_odds_mkt = $this->db->get('odds_mkt_horses');
                        ...
                        */
                        ...
                        // TB mercados
                        $where_mkt = array('id_mkt' => $mkt->id_mkt);
                        $qr_mkt = $this->db->get('mercados_horses');
                        ...
                        ?>
                            <tr>                                            
                                ...
                                <td>
                                  <input type="checkbox" value="<?=$mkt->id_mkt?>_<?=$mkt->id_mkt?>" name="runs[]" title="<?=$mkt->id_mkt?>" />
                                </td>
                                <td><?=$mkt->local?></td>
                                <td><?=$mkt->id_mkt?></td>
                                <td> <? #=$mkt->id_mkt?> <? #=$this->padrao_model->get_by_id($user->patrocinador,'user')->row()->login?>
                                  <a href="https://www.betfair.com/exchange/plus/horse-racing/market/<?=$mkt->id_mkt?>" target="_blank">
                                  <?
                                ...
                                <td>
                                    <?=$this->padrao_model->converte_data(substr($mkt->inicio,0,10))?> <?=substr($mkt->inicio,10,10)?> 
                                </td>
                                ...
                                <td class="min-width nowrap">
                                 <a href="<?=base_url()?>horse_bot_hist/rem_mkt/<?=$mkt->id_mkt?>" style='color:red'> Remover </a>
                                </td>
                            </tr>
                        <? } ?>    
                     </tbody>
...
$(document).ready(function() {
  ...
  <? if($this->session->userdata('id')){ ?>
    setInterval(function(){ 
      ...
      // horses/get_hots
      $.get("<?=base_url()?>botbet/get_odds_mkt_horses/0" , function(data){
        //alert("opa");
    ...
    setInterval(function(){ 
      ...
      // cron/get_odds_horses/1/1
      // horses/get_hots
      $.get("<?=base_url()?>horse_bot_hist/set_ordem_horses/" , function(data){
        //alert("opa");
       // $("#display_call").html(data);
      })
