      ...
      <div class="container">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb mb-0 bg-white">
              ...
              <li class="breadcrumb-item"><a href="<?=base_url()?>horses/next">cORRIDAS</a></li>
...
<!--CARD MODELO -->
           ...
           <div class="col-lg-12 col-sm-12 col-md-12 order-xl-3 order-md-3 order-sm-2 pl-1 pr-1 text-justify">
                    <div class="card mt-3 border-white">
                      ...
                      <? if($status == '0'){ ?>
                            <h1>Mercados Abertos
                            <a class="nav-link " href="<?=base_url()?>bot/horses/<?=$mkt?>/1" id="tit_mkt_now" style="font-weight: bold;font-size: 16px"> <span class="label label-success"></span>Ir para Fechados</a>
                            </h1>
                          <? } ?>

                          <? if($status == '1'){ ?>
                            <h1>Mercados Fechados
                            <a class="nav-link " href="<?=base_url()?>bot/horses/<?=$mkt?>/0" id="tit_mkt_now" style="font-weight: bold;font-size: 16px"> <span class="label label-warning"></span>Ir para Abertos</a>
                            </h1>
                          <? } ?>
                                ...
                                <? if($hots->num_rows() > 0){ ?>
                                  <? foreach($hots->result() as $dd){ $matched = 0; ?>
                                        ...
                                        $hot_volume = $dd->tamanho; 
                                            ...
                                            $qr_selecao = mysql_query("SELECT * FROM odds_mkt_horses WHERE id_mkt = '".$hot_id_mkt."' AND selection_id = '".$hot_selection."' AND selection_name <> '' LIMIT 1");
                                        ...
                                        $qr_selecao_bk = mysql_query("SELECT * FROM odds_mkt_horses WHERE id_mkt = '".$hot_id_mkt."' AND selection_id = '".$hot_selection."' AND selection_name <> '' LIMIT 1");
                                        ...
                                        $qr_evento = mysql_query("SELECT * FROM runs_horses WHERE id_evento = '".$dd->id_partida."'");
                                        $hot_evento = mysql_fetch_assoc($qr_evento);
                                        
                                        $qr_mercado = mysql_query("SELECT * FROM runs_horses WHERE marketId = '".$hot_id_mkt."'");
                                        ...
                                        if($dd->view == '0'){ 
                                          ...
                                          $this->db->update('odds_hot_horses' , $dd_view);
                                          $bg_tr = '#ffb900'; 
                                        ...
                                        <tr>
                                          <td style="font-size:">
                                            ...
                                            <? if($this->session->userdata('id')){ ?>
                                              <br>
                                              <a href="<?=base_url()?>bot/rem_hots_a2_horses/<?=$dd->id?>/<?=$mkt?>" target="_blank">
                                              Deletar</a>
                                            <? } ?>
                                          </td>
