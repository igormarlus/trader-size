...
<title>Trader Size - Unilevel</title>
...
<div class="row">
    <div class="col-md-12">
        ...
        <? $total = 0;  foreach($diretos->result() as $dd_user){ $total++; ?>
        <tr class="odd gradeA">
            <td><?=$this->padrao_model->converte_data(substr($dd_user->dt,0,10))?></td>
            <td><?=$dd_user->nome?></td>
            <td><?=$dd_user->login?></td>
            <td><?=$dd_user->plano?></td>
        	...
        	<?  
			$where_patr = array('patrocinador' => $dd_user->id , 'status' => '1');
			$this->db->where($where_patr);
			$qr_verifica = $this->db->get('user');
			if($qr_verifica->num_rows() > 0){
				foreach($qr_verifica->result() as $dd_2nivel){ $total++;
			?>
            
            		<tr class="odd gradeA">
                        <td><?=$this->padrao_model->converte_data(substr($dd_2nivel->dt,0,10))?></td>
                        <td> -- <?=$dd_2nivel->nome?></td>
                        <td> -- <?=$dd_2nivel->login?></td>
                        <td><?=$dd_2nivel->plano?></td>
                        <td class="center">Ativo</td>
                    </tr>
               <? } ?>     
