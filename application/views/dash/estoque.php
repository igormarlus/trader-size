...
<title>Trader Size - Estoque</title>
...
<div class="row">
    <div class="">
        ...
        <h3 class="title-hero">
            Estoque de Licenças (<?=$licencas->num_rows()?>)
        </h3>
        <div class="example-box-wrapper">
		<? if($licencas->num_rows() > 0){ ?>
        ...
        <? $total = 0;  foreach($licencas->result() as $li){ ?>
        <tr class="odd gradeA">
            <td><?=$this->padrao_model->converte_data(substr($li->dt_cadastro,0,10))?></td>
            <td><? if($li->id_user_use == 0){ echo "Disponível"; }else{ echo $this->padrao_model->get_by_id($li->id_user_use,'user')->row()->login; }?></td>
            <td><?=$li->valor?></td>
            <td>
            <? if($li->id_user_use == 0){  $total++;
					echo "<strong style='color:green'>".$li->key_serial."<strong>"; 
				}else{ 
					echo "<strike>".$li->key_serial."</strike>"; 
				
			}
			?>
				
            </td>
            <td class="center"><? if($li->id_user_use == 0){ echo "<strong style='color:green'>Disponível</strong>"; }else{ echo "Utilizado"; }?></td>
        </tr>
        ...
        <tr class="gradeU">
            ...
            <td>-</td>
            <td>-</td>
            <td class="center"><strong style="color:black"> <?=$total?></strong></td>
            
        </tr>
        </tbody>
        </table>
        <? }else{ ?>
        <p>Nenhuma lincença disponível. </p>
        <? } ?>
        </div></div></div>
