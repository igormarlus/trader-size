...
#			redirect('bet/hots');
	...
	function set_classificacao(){
		...
		foreach($qr->result() as $dd){
			...
			$this->db->update('cavalos_hist_dia' , $dd_update);
		}
	...
	function set_dados_run(){
		...
		if($qr_hist->num_rows() > 0){
			...
			$this->db->where('nome',$this->input->post('piso'));
			$dd_piso_by_nm = $this->db->get('piso')->row();
			$going =  $dd_piso_by_nm->sigla_racingpost;
			...
			// hist dia
			$dd_hist_class = array('foi_classe' => 1);
			$where_hist_class = array('id_mkt' => $this->input->post('id_mkt') , 'hora' => $hora);
			$this->db->where($where_hist_class);
			$this->db->update('cavalos_hist' , $dd_hist_class);
...
/*
		...
		$cont1 = 0;
		$qr_runs = $this->db->query("SELECT DISTINCT id_mkt FROM odds_mkt_horses_dia WHERE foi_import = 0  ORDER BY id_mkt desc LIMIT 2000"); //  ORIGINAL

		echo "<h2>".$qr_runs->num_rows()." corridas</h2>";
		...
		foreach( $qr_runs->result() as $dd_run){ $cont1++;
			...
			$qr_mkt_sel = $this->db->query("SELECT DISTINCT selection_id,selection_name  FROM odds_mkt_horses_dia WHERE id_mkt = '".$dd_run->id_mkt."' AND selection_name <> '' AND selection_name  NOT LIKE '%.%'");
			...
			if( $qr_mkt_sel->num_rows() == 0 ){
				...
				#return false;
					...
					foreach ($qr_mkt_sel->result()  as $horse) { #$cont2++;
						...
						if($dd_run_2->countryCode == "US" ){

							$tabela = 'corridas_cavalos_usa';
							$qr_hist = $this->db->query("SELECT * FROM corridas_cavalos_usa WHERE selection_id = '".$id_sel."'");
							###########  PERCORRE GB
							if($qr_hist->num_rows() > 0){						
								...
								foreach($qr_hist->result() as $dd_h){
									...
									$dd_wher_gb = array('selection_name' => $dd_h->selection_name, 'event_id' => $dd_h->event_id);
									#$this->db->where($dd_h);
									$this->db->where($dd_wher_gb);
								...
								echo "<h1 style='color:yellow'>Sem US</h1>";
							}
						...
						if($qr_hist_ire->num_rows() > 0){	
							...
							foreach($qr_hist_ire->result() as $dd_h){
								...
								unset($dd_h->corredores);
								unset($dd_h->foi_corredores);
								$dd_wher_ire = array('selection_name' => $dd_h->selection_name, 'event_id' => $dd_h->event_id);
								print_r($dd_wher_ire);
								#$this->db->where($dd_h);
								$this->db->where($dd_wher_ire);
								$qr_tem_ire = $this->db->get('corridas_cavalos_dia'); 
								echo "<h2 style='color:darkred'>Tem em <strong>corridas_cavalos_dia (IRE):</strong> ".$qr_tem_ire->num_rows()." ------- NM SEL: ".$dd_h->selection_name." event_dt  ".$dd_h->event_dt." </h2>";
								if($qr_tem_ire->num_rows() == 0){
									$this->db->insert('corridas_cavalos_dia' , $dd_h);
									echo "<h6 style='color:darkgreen'>INSERT  --------------------******************* </h6>";										
								}else{
									echo "<h6 style='color:darkred'>Já tem --------------------******************* </h6>";										
								}
							...
							echo "<h1 style='color:yellow'>Sem IRE</h1>";
						}
	...
	function import_corridas_cavalos_dia1(){
		#$this->db->query(" TRUNCATE TABLE corridas_cavalos_dia");
		/*
		$this->load->model('data_model');
		$hoje = date("Y-m-d");
...
/*
		...
		$cont1 = 0;
		$qr_runs = $this->db->query("SELECT DISTINCT id_mkt FROM odds_mkt_horses_dia WHERE foi_import = 0  ORDER BY id_mkt asc LIMIT 2000"); //  ORIGINAL
...
#########################  BOT HORSE ######################
	...
	*/
		...
		<form enctype="multipart/form-data" action="'.base_url().'horses/upload" method="POST">
		    ...
		    <select name="pasta">
		    	...
		    	<!-- <option value="csv_aus">AUS WIN</option> -->
		    	 ...
		    	 <option value="csv_atr">USA UPLOAD</option> 
		    	<!-- <option value="thrpost">THRPOST</option> -->
		    	<option value="csv_thr">CSV THR 1</option>
		    	<option value="csv_thr_segundo">CSV THR 2</option>
	...
	function upload_cavalos_hist(){
		...
		<h1>RESULTADO RACINGPOST - "GB/IRE"</h1>
		...
		<h1>RESULTADO ATTHERACE - "USA"</h1>
		<form enctype="multipart/form-data" action="'.base_url().'horses/import_csv_cavalo_hist_atr" method="POST"  target="_blank">
		    <!-- MAX_FILE_SIZE deve preceder o campo input -->
		...
		<h1>UPLOAD RACINGPOST "OR"</h1>

		<form enctype="multipart/form-data" action="'.base_url().'horses/import_csv_cavalo_or" method="POST"  target="_blank">
		    <!-- MAX_FILE_SIZE deve preceder o campo input -->
	...
	###### IMPORTAÇÃO JOCKEY
	function upload_jockey(){
		echo ' 
		<form enctype="multipart/form-data" action="'.base_url().'horses/upload_jockey" method="POST">
		    <!-- MAX_FILE_SIZE deve preceder o campo input -->
		    <select name="pasta">
		    	<option value="csv_jockey">Jockey GB/IRE</option>		    	
		    	<option value="csv_jockey_usa">Jockey USA</option>
		    </select>
		    Enviar esse arquivo: <input name="userfile" type="file" />
		    <input type="submit" value="Enviar arquivo" />
		</form>
	...
	function csv_ler_jockeys($pasta="csv_jockey"){
		$meuArray = Array();
	
		// MANUPUILANDO DIRETORIOS		
		#$path = 'csv_thr/';
		$path = $pasta."/"; // csv_jockey

		if($pasta == "csv_jockey"){
			$tabela = "jockeys";
		}

		if($pasta == "csv_jockey_usa"){
			$tabela = "jockeys_usa";
		}
			      ...
			      for ($col = 0; $col < $num; $col++)
			      {
			          ...
			          if($reg > 0){
				          ...
				          if($cont_cel == 1){
				          	...
				          	$dd_insert['nome'] = $nm_horse;
				          	$dd_where['nome'] = $nm_horse;
				          	
				          }
				          if($cont_cel == 2){
				          	$dd_insert['qtd_corridas'] = $data[$col];
				          	#$mes = $data[$col];
				          ...
				          if($cont_cel == 3){
				          	$dd_insert['vitorias'] = $data[$col];
				          	
				          }
				          if($cont_cel == 4){
				          	$dd_insert['vitorias_place'] = $data[$col];
				          }
			          ...
			          if($cont_cel == 4){
			          	...
			          	if($reg > 0 && $dd_insert['nome'] != '------------------------------') {
			          		$this->db->where($dd_where);
			          		// corridas_cavalos_gb
			          		$qr_row = $this->db->get($tabela);
			          		if($qr_row->num_rows() == 0){
					          	$this->db->insert($tabela , $dd_insert);		
					          	echo "<h1 style='color:blue'>DEU INSERT</h1>";			          	
					        }else{
					        	$this->db->where($dd_where);
				          		$qr_row = $this->db->update($tabela,$dd_insert);
	...
	######### IMPORTAÇÃO TREINADORES
	function upload_treinadores(){
		echo ' 
		<form enctype="multipart/form-data" action="'.base_url().'horses/upload_treinadores" method="POST">
		    <!-- MAX_FILE_SIZE deve preceder o campo input -->
		    <select name="pasta">
		    	<option value="csv_treinadores">Treinadores GB/IRE</option>		    	
		    	<option value="csv_treinadores_usa">Treinadores USA</option>
		    </select>
		    Enviar esse arquivo: <input name="userfile" type="file" />
		    <input type="submit" value="Enviar arquivo" />
		</form>
	...
	function csv_ler_treinadores($pasta="csv_treinadores"){
		$meuArray = Array();
		...
		if($pasta == "csv_treinadores"){
			$tabela = "treinadores";
		}

		if($pasta == "csv_treinadores_usa"){
			$tabela = "treinadores_usa";
		}
			          ...
			          if($cont_cel == 16){
						...
						echo "</p>";
			          	...
			          	if($reg >= 0 && $dd_where['cavalo'] != '------------------------------') {
			          		...
			          		echo "<br> TEM hist:  ".$qr_insert->num_rows()." <hr>";
				          	...
				          	*/
				          	...
				          	## hist_dia
				          	$this->db->where($where_insert);
			          		$qr_insert = $this->db->get('cavalos_hist_dia');
			          		if($qr_insert->num_rows() == 0){
			          			#$this->db->insert('cavalos_hist' , $dd_insert);
			          		}else{
			          			$this->db->where($dd_where);
			          			$this->db->update('cavalos_hist_dia' , $dd_insert);
			          		}
	...
	function import_csv_cavalo_hist_atr(){
		$meuArray = Array();
		$arq = $_FILES['userfile']['tmp_name'];
		echo $arq;
		print_r($_FILES);
	...
	// import_csv_cavalo_or
	function import_csv_cavalo_or(){
		$meuArray = Array();
		$arq = $_FILES['userfile']['tmp_name'];
		echo $arq;
		print_r($_FILES);
...
function separa_y_horse(){
	...
	$qr = $this->db->query("select DISTINCT classe from cavalos_hist ORDER BY id_mkt asc");
	#comentado por augusto
	...
	foreach($qr->result() as $dd){
		...
		if (mb_strpos($dd->classe, 'y') !== false) {
			...
			$this->db->where('classe',$dd->classe);
			$this->db->update("cavalos_hist_dia" , $new_y);
		}
...
function separa_y_horse1(){

	$qr = $this->db->query("select DISTINCT classe from cavalos_hist ORDER BY id_mkt desc");
	#comentado por augusto
	#$qr = $this->db->query("select DISTINCT classe from cavalos_hist WHERE idade IS NULL and (classe IS NOT NULL and classe <> '')");
	foreach($qr->result() as $dd){
		#$idade = explode("y", $dd->tipo);
...
function resolve_classe(){
	...
	foreach($qr->result() as $dd){
		...
		$qr_hist = $this->db->query("SELECT *  FROM cavalos_hist_dia WHERE classe LIKE '%".$dd->sigla."%' AND foi_classe = 0 and classe IS NOT NULL ORDER BY id_mkt asc");
		...
		if($qr_hist->num_rows() > 0){
			...
			foreach($qr_hist->result() as $dd2){
				...
				$this->db->query("UPDATE cavalos_hist_dia SET  classe = '".$dd->id_classe."' , classe_ = '".$dd2->classe."' , foi_classe = 1 WHERE classe LIKE '%".$dd->sigla."%' ");
	...
	$qr_hist_rem = $this->db->query("SELECT *  FROM cavalos_hist_dia WHERE classe <> 'L' ".$where_rem." AND foi_classe = 0  AND classe IS NOT NULL ORDER BY id_mkt asc");
	echo "<br><hr>";
	...
	foreach($qr_hist_rem->result() as $dd_rem){
		...
		$this->db->update('cavalos_hist_dia' , $dd_new_class);
	...
	echo '
		...
		// AYAAH.wav -------------  type="audio/mp3"
		echo '
		<audio id="audio"  src="'.base_url().'audios/AYAAH.wav"></audio>
		<audio id="audio1"  src="'.base_url().'audios/sino1.mp3"></audio>
		';
		echo "OK audio";
...
function resolve_classe1(){
	...
	foreach($qr->result() as $dd){
		...
		#augusto adicionou classe IS NOT NULL
		$qr_hist = $this->db->query("SELECT *  FROM cavalos_hist WHERE classe LIKE '%".$dd->sigla."%' AND foi_classe = 0 and classe IS NOT NULL ORDER BY id_mkt desc");
		#$qr_hist = $this->db->query("SELECT *  FROM cavalos_hist WHERE id = 11099 ");
		#$qr_hist = $this->db->query("SELECT *  FROM cavalos_hist WHERE id = 18054 ");
		if($qr_hist->num_rows() > 0){
			...
			foreach($qr_hist->result() as $dd2){
				#$this->db->query("UPDATE cavalos_hist SET  classe = '".$dd->sigla."' , classe_ = '".$dd2->classe."' , foi_classe = 1 WHERE classe LIKE '%".$dd->sigla."%' ");
				$this->db->query("UPDATE cavalos_hist SET  classe = '".$dd->id_classe."' , classe_ = '".$dd2->classe."' , foi_classe = 1 WHERE classe LIKE '%".$dd->sigla."%' ");
	...
	$qr_hist_rem = $this->db->query("SELECT *  FROM cavalos_hist WHERE classe <> 'L' ".$where_rem." AND foi_classe = 0  AND classe IS NOT NULL ORDER BY id_mkt desc");
	echo "<br><hr>";
	echo "<h1>".$qr_hist_rem->num_rows()."</h1>";		
	echo "<br><hr>";
	foreach($qr_hist_rem->result() as $dd_rem){
...
function set_cavalos_dia_em_cavalos(){
		...
		$qr = $this->db->query("select * from cavalos_dia WHERE foi_dia = 0 LIMIT 700");
		...
		if($qr->num_rows() > 0){
			...
			foreach($qr->result() as $dd){ $contador++;
				...
				$dd_foi_dia = array('foi_dia' => 1);
				$this->db->where('id',$dd->id);
				$this->db->update('cavalos_dia' , $dd_foi_dia);
	...
	function resolve_classe_vazia(){
		...
		$this->db->query("UPDATE cavalos_hist_dia SET foi_classe = REPLACE(foi_classe, 0, 1) WHERE classe = '' or classe = 'L'");
	...
	function update_place_win_cavalo(){


		$this->db->query("UPDATE cavalos_dia SET  foi_win = REPLACE(foi_win, '1', '0')");
		$this->db->query("UPDATE runs_dd_horses_dia SET  foi_place = REPLACE(foi_place, '1', '0')");
		

	}

	function replace_win_place(){


		$this->db->query("UPDATE runs_dd_horses_dia SET foi_place = 0;");
		$this->db->query("UPDATE cavalos_dia SET foi_win = 0;");
		

	}


	function replace_cavalos_hist_dia(){


		$this->db->query("UPDATE `wwtrad_ts`.`runs_horses` SET `foi_hist_2`=0;");
		

	}


	function replace_thr(){


		$this->db->query("UPDATE `wwtrad_ts`.`runs_horses` SET `foi_thr`=0;");
		

	}
	

	function replace_event_id(){


		$this->db->query("UPDATE cavalos_hist SET  foi_event_id= REPLACE(foi_event_id, 1, 0) where event_id = '0';");
	...
	function limpa_data_vazio(){
		...
		$this->db->query("UPDATE cavalos_hist_dia SET  foi_event_id= REPLACE(foi_event_id, 1, 0) where event_id = '0';");
	...
	function replace_corredores_nulos(){
		...
		$this->db->query("UPDATE cavalos_hist SET  foi_corredores= REPLACE(foi_corredores, 1, 0) where event_id = '0' and corredores is null;");
		$this->db->query("UPDATE cavalos_hist SET  foi_corredores= REPLACE(foi_corredores, 1, 0) where event_id > '303' and corredores is null;");
	...
	function replace_win_place_corredores(){
		...
		$this->db->query("UPDATE cavalos_hist SET  foi_event_id_place= REPLACE(foi_event_id_place, 1, 0) where classificacao < '5' and win_place = '0' and corredores is null;");
	...
	function replace_bsp(){
		...
		$this->db->query("UPDATE cavalos_hist SET  foi_bsp= REPLACE(foi_bsp, 1, 0) where event_id > '303' and bsp = '0.00';");
	...
	function limpeza_geral(){
		...
		$this->db->query("TRUNCATE TABLE corridas_cavalos_dia");
		$this->db->query("TRUNCATE TABLE odds_mkt_horses_dia");
		$this->db->query("TRUNCATE TABLE odds_mkt_horses_hist");
		...
		$this->db->query("TRUNCATE TABLE treinadores_hist_dia");
		$this->db->query("TRUNCATE TABLE runs_horses");
		$this->db->query("TRUNCATE TABLE confrontos");
