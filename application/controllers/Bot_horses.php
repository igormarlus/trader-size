...
class Bot_horses extends CI_Controller {
	...
	function index(){
		...
		$qr_win_fim = $this->db->query("SELECT * FROM runs_horses WHERE countryCode <> 'AV'  AND status = 2 ORDER BY marketId");
		
		$dados['qr_win_fila'] = $qr_win_fila;
		$dados['qr_win_fim'] = $qr_win_fim;
		
		$this->load->view('bot_horses/novo-index' , $dados);
		//$this->load->view('2019/novo-index-estatico' , $dados);
	}

	function get_bsp($qtd=300){
		...
		$dados['qr_run'] = $qr_run;
		$this->load->view('augusto/get_bsp', $dados);
	} // x fn

	function get_bsp_fila($vez=0){
		$dados["vez"] = $vez;
		$this->load->view('augusto/get_bsp_fila', $dados);
	} // x fn
	...
	function set_ordem_horses(){
		$qr_distinct = $this->db->query("SELECT DISTINCT event_id FROM corridas_cavalos_2020 WHERE ordem = 0 LIMIT 500");
