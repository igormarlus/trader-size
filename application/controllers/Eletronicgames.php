...
class Eletronicgames extends CI_Controller {
	...
	############  JOGADORES #################	
	function cadastro_jogadores($cb='') {
		if(!$this->session->userdata('id')){
			redirect('home/login');
		}
		...
		$dados['cb'] = $cb;
		$dados['dd'] = $dd;
		$dados['meus'] = $this->padrao_model->get_by_matriz('id_user',$this->session->userdata('id'),'EG_jogadores');
 		#$this->load->view('bo/conta' , $dados);	
		$this->load->view('eg/cadastro_jogadores' , $dados);	
	}
	function set_jogador(){
		...
		$this->db->insert('EG_jogadores' , $dd);
		redirect('eletronicgames/cadastro_jogadores');
	}

	###########  PARTIDAS ##############
	function cadastro_partidas($cb='') {
		if(!$this->session->userdata('id')){
			redirect('home/login');
		}
		$dd = $this->padrao_model->get_by_id($this->session->userdata('id'),'user')->row();
		...
		$dados['dd'] = $dd;
		$dados['meus'] = $this->padrao_model->get_by_matriz('id_user',$this->session->userdata('id'),'EG_jogadores');
		$dados['partidas'] = $this->padrao_model->get_by_matriz('id_user',$this->session->userdata('id'),'EG_partidas');
 		#$this->load->view('bo/conta' , $dados);	
		$this->load->view('eg/cadastro_partida' , $dados);	
	}
	function set_partida(){
		#print_r($_POST);
		#return false;

		$id_jogador_home = $this->input->post('id_jogador_home');
		$id_jogador_away = $this->input->post('id_jogador_away');
		#$id_vencedor = $this->input->post('id_vencedor');
		
		$gol_home = $this->input->post('gol_home');
		$gol_away = $this->input->post('gol_away');

		$resultado = $gol_home." x ".$gol_away;


		
		$dd = array(
			'id_user' => $this->session->userdata('id'),
			'id_jogador_home' => $id_jogador_home,
			'id_jogador_away' => $id_jogador_away,
			#'id_vencedor' => $id_vencedor,
			'gol_home' => $gol_home,
			'gol_away' => $gol_away,
			'resultado' => $resultado
		);
		$this->db->insert('EG_partidas' , $dd);
		redirect('eletronicgames/cadastro_partidas');
	}

	###########  CALCULADORA ##############
	function calculadora($tipo="mo",$cb='') {
		if(!$this->session->userdata('id')){
			redirect('home/login');
		}
		$id_jogador = 0;
		...
		if(isset($_POST['id_jogador'])){
			...
			$qr = $this->db->query("SELECT * FROM EG_partidas WHERE id_jogador_home = $id_jogador OR id_jogador_away = $id_jogador ");
			$dados['partidas'] = $qr;

		}
		$dados['id_jogador'] = $id_jogador;
 		#$this->load->view('bo/conta' , $dados);	
		$this->load->view('eg/calculadora' , $dados);	
	}
