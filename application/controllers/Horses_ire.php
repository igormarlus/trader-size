...
class Horses_ire extends CI_Controller {
...
#			redirect('bet/hots');
	...
	function favoritos($ordem=1){
		...
		#$qr = $this->db->query("SELECT * FROM corridas_cavalos_2022");
		#echo $qr->num_rows();
		#return false;
			      ...
			      for ($col = 0; $col < $num; $col++)
			      {
			          ...
			          if($cont_cel == 17){
			          	...
			          	if($reg > 0 && $dd_insert['selection_name'] != '------------------------------') {
			          		...
			          		// corridas_cavalos_ire
							$qr_row = $this->db->get("corridas_cavalos_ire_2022");
			          		if($qr_row->num_rows() == 0){
					          	$this->db->insert('corridas_cavalos_ire_2022' , $dd_insert);					          	
					        }				          	
				        }
			  ...
			  while (($data = fgetcsv($handle, null, ",")) !== FALSE)
			  {
			      ...
			      for ($col = 0; $col < $num; $col++)
			      {
			          ...
			          if($cont_cel == 17){
			          	...
			          	if($reg > 0 && $dd_insert['selection_name'] != '------------------------------') {
				          	$this->db->insert('corridas_cavalos_ire_place_2022' , $dd_insert);
				          	unlink($path."".$file->getFileName());
				        }
	...
	function set_ordem_horses(){
		$qr_distinct = $this->db->query("SELECT DISTINCT event_id FROM corridas_cavalos_ire_2022 WHERE ordem = 0 LIMIT 50000");
		...
		if($qr_distinct->num_rows() > 0){
			foreach($qr_distinct->result() as $corrida){
				...
				$qr_horses = $this->db->get('corridas_cavalos_ire_2022');
				if($qr_horses->num_rows() > 0){
					...
					foreach($qr_horses->result() as $horse){ $numero++;
						...
						$this->db->update('corridas_cavalos_ire_2022',$dd_numero);
	...
	function set_ordem_horses_mae_ire(){
		$qr_distinct = $this->db->query("SELECT DISTINCT event_id FROM corridas_cavalos_ire WHERE ordem = 0 LIMIT 50000");
		echo "<h1>".$qr_distinct->num_rows()."</h1>";
		if($qr_distinct->num_rows() > 0){
			foreach($qr_distinct->result() as $corrida){
				$this->db->where('event_id',$corrida->event_id);
				$this->db->order_by('bsp','asc');
				$qr_horses = $this->db->get('corridas_cavalos_ire');
				if($qr_horses->num_rows() > 0){
					$numero = 0;
					echo "<ul>";
					foreach($qr_horses->result() as $horse){ $numero++;
						...
						$this->db->where('id',$horse->id);
						$this->db->update('corridas_cavalos_ire',$dd_numero);
						echo "<li> (<b>$numero</b>) ".$horse->selection_id." - ".$horse->selection_name." <strong> @".$horse->bsp." </strong> </li>";
					}
					echo "</ul>";
				}
	...
	function set_data(){
		#echo "OK!";
		#return false;
		$qr = $this->db->query("SELECT * FROM corridas_cavalos_ire_2022 WHERE data_evento IS NULL ORDER BY id asc LIMIT 50000 ");
		...
		foreach($qr->result() as $dd){
			...
			if($this->db->update('corridas_cavalos_ire_2022',$dd_data)){
				echo "foooooi";
			}else{
				echo "naaaaaaaao";
			}
	...
	function set_data_mar_ire(){
		#echo "OK!";
		#return false;
		$qr = $this->db->query("SELECT * FROM corridas_cavalos_ire WHERE data_evento IS NULL ORDER BY id asc LIMIT 50000 ");
		#$qr = $this->db->query("SELECT * FROM corridas_cavalos WHERE datahora_evento IS NULL AND (data_evento BETWEEN '2019-01-01' AND '2019-12-31')   ORDER BY data_evento desc LIMIT 50000 ");
		#$qr = $this->db->query("SELECT * FROM corridas_cavalos_2022 WHERE data_evento = '0000-00-00' LIMIT 50 ");
		foreach($qr->result() as $dd){
			$data = substr($dd->event_dt,0,10);
			...
			echo "<br>";
			$this->db->where('id',$dd->id);
			if($this->db->update('corridas_cavalos_ire',$dd_data)){
				echo "foooooi";
			}else{
				echo "naaaaaaaao";
			}
	...
	function replace_name_tb_mae_ire($tb="corridas_cavalos_ire"){
		$this->db->query("delete from corridas_cavalos_ire where selection_name like '%lengths%'");
		$this->db->query("delete from corridas_cavalos_ire where event_name = 'Reverse FC'");
		$this->db->query("delete from corridas_cavalos_ire where event_name = 'Forecast'");
		$this->db->query("delete from corridas_cavalos_ire where event_name = 'Reverse Forecast'");
		$this->db->query("delete from corridas_cavalos_ire where event_name = 'Rrverse FC'");
		$this->db->query("delete from corridas_cavalos_ire where selection_name like '%£%'");
		$this->db->query("delete from corridas_cavalos_ire where event_name like '%Forecsat%'");
		$this->db->query("delete from corridas_cavalos_ire where selection_name = 'yes'");
		$this->db->query("delete from corridas_cavalos_ire where selection_name = 'no'");
...
function set_mae(){
		$qr = $this->db->query("SELECT * FROM corridas_cavalos_ire_2022  WHERE foi = 0 LIMIT 100000");
		echo $qr->num_rows();
		echo "<br>";
		foreach($qr->result() as $dd){
			unset($dd->id);
			...
			$this->db->where($where);
			$qr_mae = $this->db->get("corridas_cavalos_ire");
			if($qr_mae->num_rows() > 0){
				echo "Ja tem";
			}
			echo "<h2>".$qr_mae->num_rows()."</h2>";
			if($qr_mae->num_rows() == 0){
				$this->db->insert('corridas_cavalos_ire',$dd);
			}

			$foi = array('foi' => 1);
			$this->db->where($where);
			$this->db->update('corridas_cavalos_ire_2022' , $foi);
