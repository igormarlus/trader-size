...
class Horses_aus extends CI_Controller {
...
#			redirect('bet/hots');
			      ...
			      for ($col = 0; $col < $num; $col++)
			      {
			          ...
			          if($cont_cel == 17){
			          	...
			          	if($reg > 0 && $dd_insert['selection_name'] != '------------------------------') {
				          	$this->db->insert('corridas_cavalos_aus_2020' , $dd_insert);
	...
	function set_ordem_horses(){
		$qr_distinct = $this->db->query("SELECT DISTINCT event_id FROM corridas_cavalos_aus_2020 WHERE ordem = 0 LIMIT 500");
		...
		if($qr_distinct->num_rows() > 0){
			foreach($qr_distinct->result() as $corrida){
				...
				$qr_horses = $this->db->get('corridas_cavalos_aus_2020');
				if($qr_horses->num_rows() > 0){
					...
					foreach($qr_horses->result() as $horse){ $numero++;
						...
						$this->db->update('corridas_cavalos_aus_2020',$dd_numero);
	...
	function set_data(){
		#echo "OK!";
		#return false;
		$qr = $this->db->query("SELECT * FROM corridas_cavalos_aus_2020 WHERE data_evento IS NULL ORDER BY id asc LIMIT 5000 ");
		...
		foreach($qr->result() as $dd){
			...
			if($this->db->update('corridas_cavalos_aus_2020',$dd_data)){
				echo "foooooi";
			}else{
				echo "naaaaaaaao";
			}
