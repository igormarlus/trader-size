...
function set_data(){
		...
		foreach($qr->result() as $dd){
			...
			if($this->db->update('corridas_cavalos_2022',$dd_data)){
				echo "foooooi";
			}else{
				echo "naaaaaaaao";
			}
	...
	function set_ordem_horses(){
		...
		if($qr_distinct->num_rows() > 0){
			foreach($qr_distinct->result() as $corrida){
				...
				$qr_horses = $this->db->get('corridas_cavalos_2022');
				if($qr_horses->num_rows() > 0){
					...
					foreach($qr_horses->result() as $horse){ $numero++;
						...
						$this->db->update('corridas_cavalos_2022',$dd_numero);
	...
	function csv_racingpost_segundo(){
		$meuArray = Array();
	...
	function verifica_hist_e_hist_dia(){
		$qr = $this->db->query("SELECT DISTINCT selection_id,cavalo,pista FROM cavalos_hist_dia");
		#$qr2 = $this->db->query("SELECT DISTINCT cavalo FROM cavalos_hist_dia");
		echo $qr->num_rows()." - ".$qr->num_rows();
		echo "<br>";
		$cont = 0;
		foreach($qr->result() as $dd){
			#$qr_runs = $this->db->query("SELECT * FROM runs_horses WHERE marketId = '".$dd->id_mkt."'");
			$qr_runs = $this->db->query("SELECT * FROM pistas WHERE nome = '".$dd->pista."' OR nome_thr_post = '".$dd->pista."' OR sigla_racingpost1 = '".$dd->pista."' OR sigla_racingpost2 = '".$dd->pista."' OR sigla_racingpost3 = '".$dd->pista."' ");
			$qr_hist = $this->db->query("SELECT * FROM corridas_cavalos_dia WHERE selection_id = '".$dd->selection_id."'");
			if($qr_runs->num_rows() > 0){
				$dd_pista = $qr_runs->row();
				if($dd_pista->pais == "GB" || $dd_pista->pais == "IRE"){ $cont++;
					// corridas_cavalos_gb corridas_cavalos_ire
					$qr_gb = $this->db->query("SELECT * FROM corridas_cavalos_gb WHERE selection_id = '".$dd->selection_id."'");
					$qr_ire = $this->db->query("SELECT * FROM corridas_cavalos_ire WHERE selection_id = '".$dd->selection_id."'");
					$qr_gb_ire = $qr_gb->num_rows() + $qr_ire->num_rows();
					echo "[".$qr_runs->num_rows()."]".$dd->pista." - ".$dd->selection_id." - ".$dd->cavalo." ----------------------- ";
					if($qr_hist->num_rows() < $qr_gb_ire ){
						echo "Hist: <strong style='color:red'>".$qr_hist->num_rows()."</strong> - GB+IRE <strong style='color:red'>".$qr_gb_ire." [".$dd_pista->pais."] </strong>";
					}else{
						echo "Hist: <strong style=''>".$qr_hist->num_rows()."</strong> - GB+IRE <strong style=''>".$qr_gb_ire." [".$dd_pista->pais."] </strong>";						
					}
