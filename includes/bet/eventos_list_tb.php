        ...
        <tr class="odd gradeA">
			...
			######### INSERI NO BD
			$qr_num = mysql_query("SELECT id_evento FROM partidas WHERE id_evento = '".$jogo->event->id."'"); 	
			$num = mysql_num_rows($qr_num);
			...
			if(mysql_num_rows($qr_num) == 0){
				mysql_query("INSERT INTO `partidas` (`id_evento`,`evento`,`id_competition`,`inicio`) VALUES ('".$jogo->event->id."','".$jogo->event->name."' , '".$meu->id_competicao."' , '".$data_eve.' '.$diff->format( '%H:%i:%S' )."')");
			}else{
			...
			?>
                ...
                <? #=$data_eve?>
                <? #=$diff->format( '%H:%i:%S' )?>
                <span title="<?=$jogo->event->timezone?>"><?=$jogo->event->openDate?> <? #=$num?> </span>
            ...
            <td>
                <a target='_blank' title="<?=$jogo->event->id?>" href="<?=base_url()?>bet/jogo/<?=$jogo->event->id?>" style="color:#000;font-size:12px">
                <?=$jogo->event->name?>
                </a>
            </td>
            <td>
				<?  $dd_evento = $this->padrao_model->get_by_matriz('id_evento',$jogo->event->id,'partidas'); ?>
            	<?=$this->padrao_model->get_by_matriz('id_competition',$dd_evento->row()->id_competition,'betfair_competitions')->row()->nome?>  </td>
            <td>
	       <?=$jogo->marketCount?>
        	<?
			/*
