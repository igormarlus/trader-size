...
class Apiki_Buscape_API {
	...
	public function lomadizar( $link ) {
		
		$url = "http://sandbox.buscape.com/service/createLinks/lomadee/".$this->_applicationId."/?sourceId=".$this->_sourceId."&link1=".$link;
		
		
		#$url = sprintf( 'http://%s.buscape.com/service/%s/%s/%s/?%s' , $this->_environment , 'createLinks' , $this->_applicationId , $this->_countryCode , http_build_query( $args ) );
		...
		#return $this->_getContent( 'createLinks' , $this->validateParams( $args ) );
	}
	
	public function by_sandbox( $link_sand ) {
	...
	#################### POR IGOR ################
	protected function _getContent_by_sandbox( $url , array $args , $lomadee = false ) {
		// @codeCoverageIgnoreStart
		if (  !function_exists( 'curl_init' ) ){
			throw new RuntimeException( 'A extensão CURL do PHP está desabilitada. Habilite-a para o funcionamento da classe.' );
		}
